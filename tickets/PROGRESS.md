...
>   **NEXT (cold-build/author, superintendent to pick the order): `8x-01` szALPHA bridge (ticket ready NOW) · `8x-02` xALPHA-APR CRE
>   (under CRE-03, after 8x-01) · item 10 (deploy/wiring) · the **duration-squeeze freeze** (the remaining loss-side row).**
...
>   (contract track): item 10 (deploy/wiring), 8x bridge (`8x-01`, ticket ready). The **`DurationFreezeModule`** (trigger-B freeze)
>   is **BUILT-VERIFIED 2026-06-09** (48/48 module, 633/633 total; spec §8.2/§11-B fixed for a security-CRITICAL U-read).
...
| 8 | `szipUSD` — **the auto-sodomizer junior NAV vault** (**Baal Moloch-v3 + Zodiac**: Loot share, Safe-held multi-asset basket [zipUSD + xALPHA + zipUSD/xALPHA ICHI LP gauge-farmed on Hydrex + room for several strategies], ragequit in-kind exit, **NAV = tracked/displayed from multiple oracles** (not a redemption primitive), CRE-driven via manager-shaman + Zodiac modules, **LOCK (~30d) + FREEZE** gates). | §4.5 / §6.4 / §11 + `pending-docs/auto-sodomizer.md` + `hydrex.md` | multi-ticket REBUILD (~13: 8-Bw + 8-B1…12) | **TWO-TOKEN REDESIGN INTEGRATED 2026-06-07 (see the CURRENT-STATE banner at top — the model nouns in the left cell are OLD/superseded). NEXT = `8-B1` (substrate scaffold via `BaalAndVaultSummoner`) → `SzipNavOracle` → Exit Gate → engine 8-B5…8-B14. Author from `reports/baal-spec.md` §13 + `claude-zipcode.md`; cold-build WOOF-06 once 8-B1/oracle/Gate land.** | **PRIOR WOOF-07 + INFLOW-08 DELETED** (wrong vault: ERC-4626 convert-on-stake over EulerEarn loan-book pool shares). Substrate **Baal+Zodiac decided** (user-reversed the interim 7540 lean — don't need single-asset exit/4626 composability; agents 33–29 in plan). First-loss = **WITHHOLD at-risk zipUSD from ragequit** (not markdown/seizure); frozen capital keeps earning. Spec money-model rewrite (§5/§6.4/§11/§12/§17 + audit/1) + ticket decomposition pending — **design still being managed**. See `~/.claude/plans/don-t-be-dropping-deferrals-quirky-ocean.md` + §4.5 guardrail. |
...
| 8x-01 | **xALPHA bridge** — `szALPHA` LST wrapper + CCT `BurnMintTokenPool` (Bittensor 964 ↔ Base 8453) — feeds the M1 szipUSD farm-loop basket + the first-loss bond | §2 | build | **AUTHORED + 2-round critic-hardened + BUILD-READY 2026-06-09** (DEC-02 cleared; queued for cold-build). Subsumes the former `bridge/xalpha-bridge-impl.md` (DELETED — content folded into the ticket). Dev validates against a **stand-in test xALPHA token** (no blocking on CCT registration); real token swaps in. Feeds 8-B5/8-B6. | `tickets/bridge/8x-01-szalpha-wrapper-cct.md` |
| 8x-02 | **xALPHA-LST intrinsic-APR CRE feed** — `XAlphaAprOracle` push-cache + workflow (annualized `exchangeRate()` growth; NO treasury constant) | §8.8 | build (= the xALPHA-APR portion of **CRE-03**) | **AUTHORED + BUILD-READY 2026-06-09 → TODO (cold-build under CRE-03).** Subsumes the former `bridge/xALPHA-apr.md` (DELETED — content folded into the ticket). Depends on 8x-01 (`IXAlphaRate`) + CRE-00 scaffold; runs vs the xALPHA stand-in until the lane is live. Post-M1 incentive-APR overlay explicitly OUT OF SCOPE. | `tickets/bridge/8x-02-xalpha-apr-cre.md` |
