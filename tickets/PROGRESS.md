...
> - **Spec artifacts:** `claude-zipcode.md` = canonical (integrated). **`reports/baal-spec.md` = the build-grade companion
...
>   Spec/docs reconciled (`§4.5.1` recycle-only + §2/§17 NAV-accretion; `baal-spec`/`auto-compounder §6,§11`/`treasury §4.7`).
...
>   **item 9 `ZipRedemptionQueue` DONE 2026-06-09** (the SENIOR zipUSD→USDC par epoch queue; 44/44, 468/468 total — see the
>   item-9 row + `reports/9-report.md`). **8x bridge tickets `8x-01` + `8x-02` AUTHORED + BUILD-READY 2026-06-09**
>   (`tickets/bridge/8x-01-szalpha-wrapper-cct.md` [2-round critic-hardened, DEC-02 cleared] + `tickets/bridge/8x-02-xalpha-apr-cre.md`
>   [the xALPHA-APR portion of CRE-03]; both queued for cold-build per the harness §4). **The two `bridge/*.md` design docs
>   (`xalpha-bridge-impl.md` → 8x-01, `xALPHA-apr.md` → 8x-02) are SUBSUMED + DELETED — the tickets are now the source of truth.**
>   **8-Bx `LienXAlphaEscrow` DONE 2026-06-09** (per-lien xALPHA bond custody; 44/44, 512/512 total — see the 8-Bx row + `reports/8-Bx-report.md`; spec fix: §4.6 cohort-distributor → route-to-sidecar).
>   **`8x-01` szALPHA bridge DONE 2026-06-09** (BUILT-VERIFIED + KEPT — `SzAlpha`/`SzAlphaMirror`/`SzAlphaTokenPool` + deploy/wire + 30/30
>   bridge tests incl. a Base-fork registration against live CCT; the "8x exception" OZ seam resolved; see the 8x-01 row + `reports/8x-01-report.md`.
>   3 build-exposed ticket corrections folded into the kept code — amend the ticket).
>   **NEXT (cold-build/author, superintendent to pick the order): item 10 (deploy/wiring — now carries the 8x-01 + DurationFreezeModule +
>   8x-02 wiring obligations below) · the on-chain contract spine is now COMPLETE except item-10 (8x-02's `XAlphaAprOracle` receiver DONE
>   2026-06-09; its Go workflow + the rest of the CRE-00..05 off-chain track remain). The on-chain module set is otherwise
>   complete.** *(The `Exit auto-dump` module was REMOVED 2026-06-09 — user-directed: full ragequit is viable only for winding
>   the vault down in its entirety, so the impatient exit relies on CoW, not an auto-dump.)*
>   **CREDIT-UNION (szipUSD CoW-book exit) DONE 2026-06-09 — BUILT-VERIFIED + KEPT.** The CoW-exit rework (`credit-union.md`, harness-authored):
>   **C1** new `OffRampModule` (basket zipUSD→USDC via `ZipRedemptionQueue`, 17/17), **C2** the `validTo ≤ maxAge` NAV-freshness fence
>   **COLLAPSED into `SzipBuyBurnModule.postBid`** (superintendent-directed — no separate `ExitFulfillmentController`; the CRE already holds
>   `operator`+`windowController` and drives `postBid`/`burnFor` directly; SzipBuyBurnModule 36/36, +3 fence tests), **C3** deleted the
>   `ExitGate.processWindow` forfeit + orphans (`Claim`/`claimCount`/`_basketTokens`/exit events+errors + the hidden `test_invariant_sequence`
>   landmine; ExitGate 14/14), **C4** hard-gated `ZipRedemptionQueue.requestRedeem` to the **rq Safe** (not the module — it `exec`s through the
>   Safe; Timelock-settable `redeemController`, closes the epoch-dilution/senior-USDC griefing vector; 44/44). Spec gaps fixed FIRST in
>   `claude-zipcode.md` §6-intro/§6.4/§11/§17 (the stale liquidity-window/intent-queue/engine-Safe language → CoW-book). Code: `OffRampModule.sol`+`.t.sol`
>   (new) + ExitGate/ZipRedemptionQueue/SzipBuyBurnModule (+`.t.sol`) changes; `ExitFulfillmentController` deleted. `reports/credit-union-report.md`.
>   **Full non-fork suite green.** (A parallel agent is building `RecycleModule.divert` (Stream-2) in the same tree — no overlap with these
>   files; read per-suite, not the aggregate count, to attribute results.) NOT git-committed (superintendent decision).
>   The CRE off-chain build track (CRE-00 scaffold → CRE-01..05) is the natural next phase after item-10 deploy.
>   **`DefaultCoordinator` DONE 2026-06-09** (the loss-side orchestrator — NAV provision writer + xALPHA bond router; 63/63, 575/575 total —
>   see the DC row + `reports/DefaultCoordinator-report.md`; discharges the provision-bound + slash-driver obligations). Remaining M1 on-chain
>   (contract track): item 10 (deploy/wiring) + the `8x-02` xALPHA-APR CRE feed (8x-01 bridge is DONE). The **`DurationFreezeModule`** (trigger-B freeze)
>   is **BUILT-VERIFIED 2026-06-09** (40/40 module, 628 total; spec §8.2/§11-B fixed for a security-CRITICAL U-read).
>   8-B11 (CRE robot) + 8-B12 (monitoring)
>   are off-chain (`claude-zipcode.md §8` / `monitoring.md`). Build order: `reports/baal-spec.md §13`. *(Superintendent may re-prioritize
...
>
>   **BUILD-PHASE WIRING MADE TIMELOCK-SETTABLE — repo-wide (2026-06-09, user-ratified; 633/633 green).** Every contract now
>   carries ONE Timelock admin and EVERY cross-component wiring slot is Timelock-re-pointable (NOT immutable / NOT set-once
>   `AlreadyWired` / NOT renounce-frozen) so a redeployed oracle/safe/component is a one-call re-point, not a redeploy cascade.
>   Converted: loss (`LienXAlphaEscrow`, `DefaultCoordinator` incl. `recoveryFloor`), szipUSD stack (`SzipNavOracle`, `ExitGate`,
>   `SzipUSD`, `ZipDepositModule`, `ZipRedemptionQueue`, `SzipReservoirLpOracle`, `WarehouseAdminModule`), 8 engine modules +
>   `ReservoirBorrowGuard`, venue cluster (`ZipcodeController`, `ZipcodeOracleRegistry`, `EulerVenueAdapter`, `CREGatingHook`).
>   Exceptions: `LienCollateralToken.controller` stays immutable (per-line disposable token); EVK hooks use a manual `owner`
>   (OZ `Ownable._msgSender` collides with the hook decoder). No fund-extraction path added anywhere (no sweep/rescue/pause).
>   Spec: `claude-zipcode.md` §17 "Build phase: ONE Timelock admin…" bullet + §4.4. Memory [[oracle-replaceable-timelock-wiring]].
>   **DEFERRED to pre-prod lock-down (item-10-adjacent):** re-freeze wiring to immutable/set-once where appropriate + final
>   owner posture (renounce vs keep Timelock); item-10 deploy must `transferOwnership(timelock)` on every contract. The DC/8-Bx
>   security theses (destination-integrity, provision-bound) note where re-freezing restores the stronger guarantee.
...
| 8 | `szipUSD` — **the auto-compounder junior NAV vault** (**Baal Moloch-v3 + Zodiac**: Loot share, Safe-held multi-asset basket [zipUSD + xALPHA + zipUSD/xALPHA ICHI LP gauge-farmed on Hydrex + room for several strategies], ragequit in-kind exit, **NAV = tracked/displayed from multiple oracles** (not a redemption primitive), CRE-driven via manager-shaman + Zodiac modules, **LOCK (~30d) + FREEZE** gates). | §4.5 / §6.4 / §11 + `pending-docs/auto-compounder.md` + `hydrex.md` | multi-ticket REBUILD (~13: 8-Bw + 8-B1…12) | **TWO-TOKEN REDESIGN INTEGRATED 2026-06-07 (see the CURRENT-STATE banner at top — the model nouns in the left cell are OLD/superseded). NEXT = `8-B1` (substrate scaffold via `BaalAndVaultSummoner`) → `SzipNavOracle` → Exit Gate → engine 8-B5…8-B14. Author from `reports/baal-spec.md` §13 + `claude-zipcode.md`; cold-build WOOF-06 once 8-B1/oracle/Gate land.** | **PRIOR WOOF-07 + INFLOW-08 DELETED** (wrong vault: ERC-4626 convert-on-stake over EulerEarn loan-book pool shares). Substrate **Baal+Zodiac decided** (user-reversed the interim 7540 lean — don't need single-asset exit/4626 composability; agents 33–29 in plan). First-loss = **WITHHOLD at-risk zipUSD from ragequit** (not markdown/seizure); frozen capital keeps earning. Spec money-model rewrite (§5/§6.4/§11/§12/§17 + audit/1) + ticket decomposition pending — **design still being managed**. See `~/.claude/plans/don-t-be-dropping-deferrals-quirky-ocean.md` + §4.5 guardrail. |
| 9 | `ZipRedemptionQueue` | §4.5 / §6.1 | build | **DONE 2026-06-09 — BUILT-VERIFIED + KEPT.** The SENIOR exit: zipUSD→USDC at **par**, 30-day epoch queue, pro-rata partial fills, carry-forward, NO mid-epoch cancel. ERC-7540 lifecycle (clean-room fork of `reference/erc7540-reference`, NOT inherited — escrows EXTERNAL zipUSD + pays USDC at par, not solmate's share==this/NAV) + a Maple-style pro-rata via a **global cumulative-remaining factor scoped by an `era` counter** (O(1), iteration-free, fair/censorship-resistant — no settle caller can omit a requester). Immutable `controller` (the CRE redemption-settle operator), `onlyController settleEpoch`, **never renounced**, NO Owned/owner/sweep/pause (non-sweepable). Funded by warehouse REDEEM→REPAY (queue reads its OWN USDC balance; **calls EulerEarn never** — KR-1). **44/44** (37 unit + 5 invariant/fuzz incl. the ghost-accumulator solvency harness `ghost_totalPaid<=ghost_totalDelivered` over 128k calls + 2 Base-fork: real `WarehouseAdminModule` REPAY→settle, real `ESynth` `totalSupply` drop, KR-2 no-sweep-selector ABI negative), **468/468 total no regression** (`forge test`; full suite incl. fork — independently re-run). **ZERO load-bearing guesses.** Code at `contracts/src/supply/ZipRedemptionQueue.sol` + `contracts/src/interfaces/euler/IZipUSD.sol` + `contracts/test/ZipRedemptionQueue.t.sol`. NOT git-committed (consistent with the recent 8-B* windows — superintendent commit decision). `tickets/sodo/9-zip-redemption-queue.md`; `reports/9-report.md`. | `tickets/sodo/9-zip-redemption-queue.md` |
| 10 | Deploy + wiring script (vanilla Euler/OZ/CRE config) | §9 | build (script) | **DEPLOY SCRIPT AUTHORED + `forge build` GREEN 2026-06-10** — `contracts/script/DeployZipcode.s.sol` (625 lines, `is SummonSubstrate`, phases `_phaseP0..P9`, entrypoint `deploy()`), authored FROM `wires/` via the cold-build harness; every ctor/`setUp`/sub-deployer entrypoint verified against the kept code (zero load-bearing guesses). Deploys the full Base-side protocol in dependency order (venue spine → rate oracle → Baal two-Safe substrate via inherited `_summon` → zipUSD/depositModule/navOracle/Gate/szip/queue → warehouse via `CreditWarehouseDeployer` → reservoir market via `ReservoirMarketDeployer` → 9 engine-module clones [ModuleProxyFactory + `setUp` + `enableModule` on the right Safe(s)] → loss side → NAV wiring → identity-seal S10b → `ZipcodeDeployAsserts.requireIdentityWired` pre-gate → `transferOwnership(timelock)` everywhere, NOT renounce). Circular ctor deps resolved by the build-phase Timelock-settable setters (wires/ seam #1). All 8 cross-cutting seams asserted inline (one-bank / shared-LP / engineSafe-trio / venue-registry / zero-Shares / escrow↔coord / nav-shareToken / non-commingling). 5 build-discovered code-vs-spec deltas folded into the ticket. **REMAINING (fork-execution step, not this window):** EE-pool admin config (createEulerEarn/allocator/curator/fee + point supply queue at the reservoir borrow vault — fork-only, the local `IEulerEarn` shim avoids the 0.8.26 source; `EE_POOL`/`BASE_USDC_MARKET` are env inputs); a `test/DeployZipcode.t.sol` Phase-S fork run + the identity-pregate tested-negative + an L4 origination; the deferred `audit/2`/`audit/3` engine/junior/loss L-row sweeps; the 8x bridge two-chain wire (964, separate script). NOT git-committed (consistent with recent windows). | `tickets/woof/WOOF-10-deploy-wiring.md`; covers `audit/2.md` S1–S12 |
...
| 8-Bx | `LienXAlphaEscrow` — per-lien xALPHA bond custody (slashable reservoir) | §4.6 / §11 | build | **DONE 2026-06-09 — BUILT-VERIFIED + KEPT.** Per-lien xALPHA first-loss bond custody: a standalone, fully-immutable, non-sweepable vault (the loss-side sibling of item 9 `ZipRedemptionQueue` — `is ReentrancyGuard` only base, no owner/sweep/pause/setter). Four `onlyCoordinator nonReentrant` state-changers — `lockXAlpha(bytes32 lienId, originator, amount)` (CEI pull-in, no clobber) / `releaseXAlpha` (full bond → recorded originator on repay, M1-live) / `slashXAlphaToCapital(amount)` (partial ≤ bond → immutable `capitalSink`, off-chain alpha→TAO→USDC, mock-tested M2-live) / `slashXAlphaToCohort` (remainder → immutable `sidecar`, in-kind premium, NAV does the pro-rata, mock-tested M2-live). Modeled on `InsuranceFund.bring:33` (single-immutable-authorized-caller + gated `safeTransfer`). **Security thesis (fork-of-funds proven by a 128k-call stateful invariant): destination integrity — funds can ONLY ever reach {recorded originator, `capitalSink`, `sidecar`}, no recipient param, so even a compromised coordinator cannot steal to an attacker address** (residual = grief via premature release / slash-healthy, the coordinator's §13 trust boundary, documented). **44/44** (unit + reentrancy probe [coordinator==reentrant token → `ReentrancyGuardReentrantCall`] + 2 stateful invariants over 128k calls + fuzz), **512/512 total no regression** (`forge test` — independently re-run, fork tests incl.). **ZERO load-bearing guesses.** `bytes32 lienId` aligned to the canonical controller/CRE type (security critic catch). Code at `contracts/src/loss/LienXAlphaEscrow.sol` + `contracts/test/LienXAlphaEscrow.t.sol`. NOT git-committed (consistent with item 9 / 8-B* windows — superintendent commit decision). **Spec fix this window:** `claude-zipcode.md §4.6` cohort-distributor (the stale "snapshot → per-holder share / RewardsDistributor-style" sentence contradicted §11/§6.4 "no per-position index, no SBT" → replaced with route-to-sidecar + NAV-pro-rata; spec-fidelity-confirmed). `tickets/loss/8-Bx-lien-xalpha-escrow.md`; `reports/8-Bx-report.md`. | `tickets/loss/8-Bx-lien-xalpha-escrow.md` |
| DC | `DefaultCoordinator` — the loss-side orchestrator (NAV provision writer + xALPHA bond router; the freeze is NOT its job) | §4.6 / §11 / §7 / §8.4 | build | **DONE 2026-06-09 — BUILT-VERIFIED + KEPT.** The single loss-side orchestrator: a **CRE-gated `ReceiverTemplate`** (renounce-frozen) that **is** the immutable `LienXAlphaEscrow.coordinator` (owns the full bond lifecycle — LOCK/RELEASE M1-live, slash M2) **and** the oracle's set-once `defaultCoordinator` (sole `writeProvision` caller). Holds the launch xALPHA reserve + `forceApprove(escrow, max)`. **The load-bearing logic = the bound the oracle omits:** per-lien `(status, provision)` + `totalProvision` pushed to `SzipNavOracle.writeProvision`; **down only by `atRisk×(1−recoveryFloor)`** at recognition (recoveryFloor = immutable ctor param), **up only by realized receipts** (floored 0), **never an arbitrary NAV**. reportType-8 action family (LOCK/RELEASE/DEFAULT/RECOVERY/RESOLVE/WRITEOFF) dispatched in `_processReport`; RESOLVE/WRITEOFF drive `slashXAlphaToCapital`(if>0)→`slashXAlphaToCohort` (reads `escrow.bondAmount` to skip cohort on full-capital-slash); WRITEOFF settles the residual permanently (no `writeProvision`). Status machine forbids re-recognition / post-resolution heal / release-of-defaulted / **LOCK-on-terminal** (the escrow would allow a re-lock at `bondAmount==0` — the coordinator's terminal-status guard blocks it). Security thesis: bounds + routes, does NOT validate a default is real (the §13 CRE trust boundary documented in a NatSpec block); destinations structurally immutable (no theft except the CRE-named originator leg); NAV can never be marked above the un-impaired basket. **63/63** (57 unit + 3 integration vs the REAL `SzipNavOracle`+`LienXAlphaEscrow` + 3 stateful invariants over 128k calls/0 reverts incl. the independent `ghost_cap` security thesis + the `totalProvision==Σ==oracle.provision()` invariant), **575/575 total no regression** (independently re-run, deterministic ×2). **ZERO load-bearing guesses.** Code at `contracts/src/loss/DefaultCoordinator.sol` + `contracts/src/interfaces/loss/{ISzipNavOracle,ILienXAlphaEscrow}.sol` + `contracts/test/DefaultCoordinator.t.sol`. NOT git-committed (consistent with the recent loss-side/8-B* windows — superintendent commit decision). **Spec surgery this window:** §4.6 DefaultCoordinator bullet detailed to build-grade + §4.4 reportType-8 row added + §8.4 action ABI authored (spec-fidelity verdict FAITHFUL — detailing, no §17 reopen). `tickets/loss/DefaultCoordinator.md`; `reports/DefaultCoordinator-report.md`. | `tickets/loss/DefaultCoordinator.md` |
| — | **`DurationFreezeModule`** — Duration-Bond trigger B: the autonomous on-chain release-floor + CRE main↔sidecar rotation (the structural sidecar freeze that gates the Exit Gate's windowed exit; utilization-sized, no xALPHA premium/slash) | §11-B / §6.4 / §8.2 | build | **DONE 2026-06-09 — BUILT-VERIFIED + KEPT.** A Zodiac `Module` on BOTH Safes: `commit`(main→sidecar, ungated) + `release`(sidecar→main, gated by `committedValue ≥ requiredFraction(U)×grossBasketValue`); `requiredFraction = utilization -- freeze% = utilization%, DEAD-SIMPLE (superintendent 2026-06-09; the U_lock/U_max/maxLockFraction escalation is post-M1, not built)`; **U read donation-immune** off the senior pool (`1 − maxWithdraw(warehouse)/convertToAssets(balanceOf(warehouse))`). Bounds-not-validates (DefaultCoordinator pattern); gates the Exit Gate structurally (no ExitGate change). **Critics: spec-fidelity FAITHFUL (all 5 flagged tensions), ref-verifier all sigs OK; security found + I fixed a CRITICAL** (the v1 `idle = balanceOf(eulerEarn)` U-read was wrong [totalAssets excludes idle → U≈1, bricks release] AND donatable [lower U → lower floor → over-release run-hatch] → redesigned to the donation-immune `maxWithdraw` read + **fixed spec §8.2/§11-B**) + a HIGH (non-basket-asset leak → added the 5-leg oracle whitelist). **Cold-build (fresh subagent), ZERO load-bearing guesses, independently re-ran green (forge clean): 40/40 module** (unit + the FreezeHandler 128k-call invariant `committedValue ≥ requiredCommittedValue` post-release/0 reverts + 2 Base-fork: real `_summon` substrate + real `SzipNavOracle` cross-Safe rotation + a forced `FreezeFloorBreach`), **628/628 total no regression** (independently re-run from clean); the **`SzipNavOracle` 42-test suite + its `grossBasketValue` exact pins UNCHANGED** (the extension is additive — new `committedValue()`/`freeValue()` via a private `_grossValueOf`). Code `contracts/src/supply/szipUSD/DurationFreezeModule.sol` + `contracts/src/interfaces/{euler/IEulerEarnUtil,supply/ISzipNavBasket}.sol` + the additive `SzipNavOracle` views + `contracts/test/DurationFreezeModule.t.sol`. NOT git-committed (superintendent decision). **Build-surfaced + USER-CONFIRMED INTENDED 2026-06-09:** the branch `SzipNavOracle.sol` setters are Timelock-re-pointable (not set-once-renounce-frozen) **by design — oracles are replaceable** (deploy multiple / hot-fix one); memory [[oracle-replaceable-timelock-wiring]]. **Reconcile before item-10** (logged in Open spec gaps): the DefaultCoordinator "set-once `defaultCoordinator`, immutable after renounce" thesis → "sole writer, Timelock can re-point"; oracle-consumers holding a set-once `navOracle` (this module, the Gate) get a re-point path or re-clone on oracle swap. `tickets/sodo/DurationFreezeModule.md`; `reports/DurationFreezeModule-report.md`. | `tickets/sodo/DurationFreezeModule.md` |
| — | `GRAPH-01` event indexing — `schema.graphql` (entities + fields) + `subgraph.yaml` (datasources + the per-line dynamic-datasource template) + mapping handlers + a Matchstick unit per handler | §9 / README §348 | build (subgraph track) | **AUTHORED 2026-06-09 — `tickets/subgraph/GRAPH-01-event-indexing.md`.** Cold-buildable subgraph indexing spec, **every datasource ABI + event signature `forge inspect`-verified against the built contracts** (selectors pinned; no §9-prose hand-typing). 14 fixed datasources (controller/factory/registry/venue/deposit/redemption/Gate/freeze/NAV-oracle/LP-oracle/escrow/coordinator + 7 engine modules) + the **`LineBorrowVault` DYNAMIC TEMPLATE** spawned at `LineOpened` (the per-line isolated market — no manifest-time address). ~20 entities with `Lien` (key `lienId`) as the spine + `Line` (key `lineRef`); NAV indexed from the **oracle's OUTPUTS** (`Poked`/`LegPriceUpdated`/`ProvisionWritten` → `NavSnapshot`), **never recomputed**. **Resolved the lienId↔lineRef join (explicit, same-tx — `LienOriginated` carries both, `:79`)** + the template-vs-direct-read split + interface-vs-emitter binding. **1 contract back-pressure filed** (B-1: `ZipRedemptionQueue.Withdraw` lacks `requestId`). item-10 address/startBlock are the ONLY placeholders. **Acceptance (deferred — graph CLI not installed):** `graph codegen`+`graph build`+`graph test` green. (Prep kit consumed + deleted.) | `tickets/subgraph/GRAPH-01-event-indexing.md` |
| — | `GRAPH-02` dashboard-metric derivation — NAV / zipUSD peg / szipUSD APR / utilization / insurance-coverage formulas (+ subgraph-vs-direct-read tag, snapshot model) | §12 / README §348 / monitoring.md | build (subgraph track) | **AUTHORED 2026-06-09 — `tickets/subgraph/GRAPH-02-metric-derivation.md`.** The 5 metrics on top of GRAPH-01, each with exact input entities/view-reads + a subgraph-vs-direct-read tag (none ambiguous) + the time-series model. **NAV** = subgraph history (oracle-output series) + live `navEntry()/navExit()` direct read. **APR** = SUBGRAPH trailing-realized, **7d+30d**, anchored on the oracle's **TWAP accumulator** (`cumNav`/`oracleTs` deltas), kept distinct from the xALPHA intrinsic APR (8x-02). **utilization** = junior (`FreezeAction.committedValueAfter`÷NAV) + senior (origination throughput) series + live `DurationFreezeModule.utilization()` read. **peg** = subgraph history via a NEW external Hydrex/Algebra pool datasource (deferred to item-10, pool addr is a deploy output) + live pool read. **insurance-coverage = OFF-CHAIN / NOT-SUBGRAPH** (origination-gate-implied + CRE feed; no faked on-chain number, no back-pressure event — intentional). **Cadence = event-driven `NavSnapshot` base + daily `NavDayData` rollup.** (Prep kit consumed + deleted.) | `tickets/subgraph/GRAPH-02-metric-derivation.md` |
| 8x-01 | **xALPHA bridge** — `szALPHA` LST wrapper + CCT `BurnMintTokenPool` (Bittensor 964 ↔ Base 8453) — feeds the M1 szipUSD farm-loop basket + the first-loss bond | §2 | build | **DONE 2026-06-09 — BUILT-VERIFIED + KEPT.** Cold-built the full bridge: `SzAlpha` (964 UUPS LST wrapper — pooled-staker over the StakingV2 precompile, `deposit`/`redeem`, `IXAlphaRate.exchangeRate()`, CCT mint/burn leg, S1–S11 all covered), `SzAlphaMirror` (Base plain `BurnMintERC20`, no stake surface), `SzAlphaTokenPool` (CCT `BurnMintTokenPool` + 18-dp/canonical-RMN asserts), minimal bridge interfaces, `DeploySzAlphaBridge` (both-chain deploy + self-serve wire + the 5-address re-read battery), fork+mock test suite. **30/30 bridge** (29 unit w/ etched precompiles + **1 Base-mainnet-fork** integration that runs the REAL `registerAdminViaGetCCIPAdmin`→`acceptAdminRole`→`setPool` against live Base CCT `RegistryModuleOwnerCustom 1.6.0`/`TokenAdminRegistry 1.5.0` + asserts `getPool`). **No regression: full non-fork suite 632/632 (incl. all 29 bridge unit tests) + bridge-on-fork suite 30/30; a full `--fork-url base` run reached 37 suites/0 failures before a PRE-EXISTING 128k-call fork-invariant suite stalled on a throttled Base RPC (this window is purely additive — new files + longer-prefix remaps, `forge build` fully green — so existing fork suites are unaffected; the single grand-total line was un-obtainable this session due to RPC throttling).** **The "8x exception" OZ version seam RESOLVED** (versioned `@4.8.3`/`@5.3.0` prefixes + OZ-Upgradeable 5.1.0 context-scoped; probe-confirmed; no core bump). All 5 Base CCT addresses re-read on-chain = exact table match. **Build-exposed corrections folded into the kept code** (the ticket should be amended): (1) `registerAdminViaOwner`→`registerAdminViaGetCCIPAdmin` (the canonical `BurnMintERC20` has no `owner()` + `SzAlpha.owner()`=timelock-from-genesis → registrar is the separate `ccipAdmin`); (2) UUPS ⊥ constructor-based `BurnMintERC20` → `SzAlpha` is a FRESH OZ-Upgradeable token implementing the `IBurnMintERC20` surface, only the immutable mirror inherits the canonical; (3) "1e3 dead shares to address(0)" (impossible in OZ, wouldn't stop div-by-zero) → OZ ERC-4626 virtual-offset (genesis 1:1, donation structurally inapplicable in the pooled-staker model). Code: `contracts/src/bridge/{SzAlpha,SzAlphaMirror,SzAlphaTokenPool}.sol` + `contracts/src/interfaces/bridge/{ISubtensorPrecompiles,ICctRegistry}.sol` + `contracts/script/DeploySzAlphaBridge.s.sol` + `contracts/test/bridge/{SzAlphaBridge.t.sol,BridgeMocks.sol}` + `contracts/remappings.txt` (+6 additive lines). NOT git-committed (superintendent decision, consistent with recent windows). Flag: TAO/alpha/rao denomination + the 964-fork lane are integration-time (deploy-fixtures). `tickets/bridge/8x-01-szalpha-wrapper-cct.md`; `reports/8x-01-report.md`. | `tickets/bridge/8x-01-szalpha-wrapper-cct.md` |
| 8x-02 | **xALPHA exchange-rate Base oracle + derived APR** — `SzAlphaRateOracle` (CRE pulls the rate from 964 → Base; NAV reads `exchangeRate()`, APR derives on-chain) | §8.6/§8.8 | build (Base rate oracle + CRE rate-pull) | **DONE 2026-06-09 — BUILT-VERIFIED + KEPT (REDESIGNED ×2, user-directed, same window).** Arc: v1 push-cache received a **pre-computed APR** (over-built, adversarial bands) → v2 `XAlphaAprOracle` derived natively on 964 (right principle, wrong chain — consumers are Base-side) → **v3 FINAL: `SzAlphaRateOracle` on Base.** The one fact native only to Bittensor is the **rate** (`staked/supply`, `0x805`, 964; the Base mirror has no stake surface), so a CRE workflow **pulls the rate from 964 and pushes it RAW to Base** (`reportType RATE=8`, payload `(uint256 rate, uint48 ts)`); **CRE transports the primitive, the chain derives NAV+APR.** `SzAlphaRateOracle is ReceiverTemplate, IXAlphaRate` — the Base-side `exchangeRate()` (drop-in for `SzipNavOracle`'s xALPHA leg + an Euler adapter) + `fresh()`/`lastUpdate()` (consumers fail-closed on staleness); push guards truthful (non-zero/not-future/**strictly-newer**, NO deviation band — a slash legitimately lowers the rate); the intrinsic **APR is a DERIVED view** (`intrinsicAprBps()`, floored-0/cap-clamped, advisory). **Resolves the §8.6 cross-chain rate seam** (NAV's xALPHA rate now has a producer). **17/17 suite** (ctor guards, forwarder-gate, wrong-type/ZeroRate/FutureTimestamp/StaleReport-replay, `exchangeRate()` IXAlphaRate drop-in, fresh-flips-rate-still-served, derived APR=1216 bps, slash⇒0-not-brick, cap-clamp, APR-zero-before-warm), **675/675 total non-fork no regression** (`forge test --match-contract SzAlphaRateOracle`; full `forge test` — independently re-run). **ZERO load-bearing guesses.** **DELETED:** the v1 push-cache (Forwarder + pushed APR + deviation band + fallout) and the v2 964-native `XAlphaAprOracle`; the Go workflow is now the **rate pull** (`cre/szalpha-rate/`, raw-rate only, no math). **Spec:** §8.8 rewritten (rate→Base oracle, APR derived); §8.0 gains the `8` RATE / `SzAlphaRateOracle` row. Code `contracts/src/bridge/SzAlphaRateOracle.sol` + `contracts/test/bridge/SzAlphaRateOracle.t.sol` + `cre/szalpha-rate/{main.go,README.md}`. NOT git-committed (superintendent decision). `reports/8x-02-report.md`. | `tickets/bridge/8x-02-xalpha-apr-cre.md` |

### Item 8 decomposition — szipUSD = the auto-compounder NAV vault (Baal + Zodiac)
The 2026-06-05 `WOOF-07`/`INFLOW-08` (ERC-4626 convert-on-stake over EulerEarn pool shares) were **DELETED — wrong
vault.** szipUSD is the **auto-compounder junior NAV vault**: a **Baal/Moloch-v3 + Zodiac** vault (Loot share;
...
| 8-B14 | **szipUSD buy-and-burn** | engine USDC posts discounted `BUY szipUSD` CoW bids **below NAV** → **burn** on fill (the impatient-exit liquidity floor + the haircut-to-stayers mechanism). `reports/baal-spec.md §7` / §4.5.1. | 8-B9, Exit Gate | **DONE 2026-06-08 — BUILT-VERIFIED.** `SzipBuyBurnModule` (bid side only; burn = existing `ExitGate.burnFor` by the windowController). `is Module` on the engine Safe, `onlyOperator`, posts a single resting CoW `BUY szipUSD` PRESIGN bid priced `≤ navExit×(1−d)`, `sellAmount≤buybackCap`, signs via on-chain GPv2 uid + `setPreSignature` (Call-only, no delegatecall), exact-`sellAmount` USDC approve. **33/33 Base-fork** (uid known-answer vector + live `preSignature`/allowance; price boundary incl. non-divisible RHS; navExit-vs-twap; cap/kill-switch/single-bid; partial-fill→cancel→repost; atomicity rollback; freshness; authority/shape; exec-discipline Call/value==0), **238/238 total no-regression** — run `forge test --fork-url $BASE_RPC_URL --match-contract SzipBuyBurnModuleTest`. Code at `contracts/src/supply/szipUSD/SzipBuyBurnModule.sol` + `contracts/src/interfaces/cow/IGPv2Settlement.sol`. CoW addrs added to `BaseAddresses.sol`. NOT git-committed (whole tree untracked). `reports/8-B14-report.md`. |

| 8x-01 | **`szALPHA` LST wrapper + CCT transport** | self-built LST over Subtensor precompiles (own validator, `SzAlpha`) + a Base mirror (`SzAlphaMirror`) + Chainlink CCT `BurnMintTokenPool` bridging 964↔8453; the real `xALPHA` leg the basket/CRE consume (stand-in until live) | (none on-chain; off critical path) | **TICKET BUILD-READY 2026-06-09** — `tickets/bridge/8x-01-szalpha-wrapper-cct.md`. **TWO critic rounds** (junior/spec-fidelity/ref-verifier/qa/security); converged, ZERO spec gaps both rounds. R1 added pooled-staker model + Base mirror + S1–S11 + tests; R2 fixed the `deposit`-vs-`mint` path split, tightened S4 (both stake dirs), pinned dead-shares/timelock/5 deploy asserts, added 6 named test skeletons. All sigs/structs/selectors/10 addresses ref-verified. No 3rd re-fan needed (R2 = tightening). DEC-02 cleared. **Queued for cold-build** (harness §4). Real-world deploy prereq only: subnet/validator registration (NETUID/VALIDATOR_HOTKEY literals are `initialize` args — build proceeds on fixtures). Off critical path — sequence against 8-B/CRE. |
...
**CRE track + DEC gates (the authoritative status pane).** The §8 producer spec is DONE (row above); the
workflows are now authorable. This pane + the spec map `claude-zipcode.md §8.11` are authoritative for the CRE
build track and the DEC gates. *(PHASE2.md, the old dormant-future backlog, was deleted 2026-06-09 when the
M1/M2 split collapsed — its live stubs were lifted here + into the backlog table above.)*

| Item | What (§) | Status / gate |
|---|---|---|
| `CRE-00` | project/secrets scaffold | **AUTHORABLE NOW** — no blocker |
| `CRE-01` | origination/draw/close/status → controller + revaluation (sharded) → registry + default → `DefaultCoordinator` (§8.1/§8.4) | spec DONE; **DEC-01 RESOLVED (two-layer model)** — builds on mock Proof+feeds, swaps real endpoints in; *live real-collateral* origination still needs insurance product (§6.2) + legal (§6.3) + feed/SPV vendors |
| `CRE-02` | redemption-settle `cron` + warehouse REDEEM (§8.3/§8.5) | spec DONE; warehouse leg waits on **8-Bw** |
| `CRE-03` | szipUSD share-price feeds `NAV_LEG`(7)/`LP_MARK`(7) + xALPHA-APR (§8.6/§8.8) | spec DONE; **DEC-02 cleared** — xALPHA lane is build-only (self-serve CCT confirmed), no decision gate |
...
| `DEC-01` | Proof per-lien attestation capability (lien/value/insurance/recovery) | **RESOLVED 2026-06-09 — two-layer model.** A notarization attests instrument genuineness/ownership/integrity, NOT the facts → facts come from authoritative feeds (recorder/title = existence + anti-fabrication, appraiser = value, carrier = insurance, Erebor = recovery) on the same CRE identical-consensus path; DON hashes the sealed artifact on-node + verifies cert; DON-only fetch. **NOT a build blocker** (CRE-01 on mock, swaps real feeds in). Residual (own risks): insurance product (§6.2) + legal (§6.3) + vendor/SPV pinning. `pending-docs/spv-lien-proof.md §6.1`. |
| `DEC-02` | xALPHA canonical-vs-fork + CCT registration (chain 964) | **RESOLVED (2026-06-09)** — (a) fork decided: self-built `szALPHA` on our own validator; (b) CCT registration on 964 confirmed **open/self-serve** (full TokenAdminRegistry+RegistryModuleOwnerCustom+TokenPoolFactory stack live; GTV xSN* tokens already registered w/ Base lane — addresses in `bridge/xalpha-bridge-impl.md §4`). CRE-03's real xALPHA lane is now build-only, no decision gate |
| `DEC-03` | GTV pairing terms (zipUSD vs USDC) | **RESOLVED 2026-06-09** — Hydrex confirmed LP pair = **xALPHA / zipUSD** (the stable unit; USDC not mandated). Confirms the spec's existing `zipUSD/xALPHA` pool (`claude-zipcode.md §4.5`, `hydrex.md`); only the `[[rubicon-fork-and-closed-loop]]` memory's "xALPHA/szipUSD" phrasing was stale. Treasury rationale re-authors post-M1 |
...
(CRE / Inflow / treasury tracks are authored when reached; this ledger leads with the M1 contract
critical path. **Subnet (Bittensor containers + Proof fetch) is owned by a separate developer — out of this
ledger.**)
...
| **the freeze/rotation module** (NOT item-9 `ZipRedemptionQueue`) · sidecar rotation/funding | do NOT fund/route value into the **sidecar** until `isOwner(team)==true` on it (8-B1's `_addOwnerToSidecar` lands it; the sidecar ships Baal-only until then) and the team has proven it can drive the sidecar. **(LABEL FIX 2026-06-09:** this "item 9" is **baal-spec §8's internal numbering for the junior freeze/rotation module** — a CRE-operated Zodiac module on both Safes, an item-10/8-B11 component — NOT the PROGRESS-backlog item 9 `ZipRedemptionQueue` (the senior zipUSD→USDC queue, which touches no sidecar). The senior queue is right to ignore this.) | 8-B1 (security F6.2) | **DISCHARGED 2026-06-09** (`DurationFreezeModule` BUILT-VERIFIED) — `commit` is the only sidecar-funding path + fails closed (`execTransactionFromModule` reverts) until the module is enabled on the team-owned sidecar; the fork test proves the enable-on-both-Safes path; item-10 enables on the sidecar only after `isOwner(team)`. |
...
| **DefaultCoordinator (§11/§4.6)** · provision bound | it is the sole `SzipNavOracle.writeProvision` caller (wired via `setDefaultCoordinator`) and MUST enforce the bound (down by `atRisk×(1−recoveryFloor)`, up by realized receipts) — the oracle stores the value **unbounded** by design. | `SzipNavOracle` | **DISCHARGED (by DefaultCoordinator, 2026-06-09)** — the coordinator is the sole `writeProvision` caller; DEFAULT sets `provision = atRisk×(1e18−recoveryFloor)/1e18` (recoveryFloor immutable, round-down), RECOVERY reduces by `min(provision, recoveryProceeds)` (floored 0), RESOLVE → 0, WRITEOFF settles permanently; the `totalProvision == Σ lienLoss.provision == oracle.provision()` + independent `ghost_cap` invariants proven over 128k calls (no arbitrary-NAV path, never above the un-impaired basket). |
| **10** · Deploy/wiring (§9) — SzipNavOracle | after deploying szipUSD/Gate, the LP/gauge, the engine Safe, and the DefaultCoordinator, call the four wiring setters (`setShareToken`/`setLpPosition`/`setEngineSafe`/`setDefaultCoordinator` — Timelock-re-pointable, §17); **assert `shareToken() != 0` before `transferOwnership(timelock)`** (else the oracle is stuck at the genesis price); **transfer ownership to the Timelock LAST, NOT renounce** (§17 build-phase: Forwarder/identity/wiring stay re-pointable; immutability deferred to pre-prod). | `SzipNavOracle` | OPEN |
...
| **`SzipNavOracle` code change + item-10** · split the xALPHA rate-source from the balance-token (8x-02 seam — superintendent 2026-06-09) | **The built `SzipNavOracle` conflates them: ONE immutable `xAlpha` (`:56`) is read BOTH as `IXAlphaRate(xAlpha).exchangeRate()` (`:381`, the rate) AND `IERC20(xAlpha).balanceOf(...)` (`:256/:295/:376`, the basket balance).** That held in M1 because the xALPHA stand-in exposed both faces. **Production splits them:** the basket holds `SzAlphaMirror` (a plain `BurnMintERC20`, NO `exchangeRate()`), and the rate comes from the separate Base `SzAlphaRateOracle` (NO `balanceOf`). So the 8x-02 plan ("point the rate read at the oracle, token stays the mirror for balances") is **NOT a wiring re-point** — `xAlpha` is immutable + single-purpose. → Add a **distinct rate source** to the oracle (a new `xAlphaRate` address read at `:381`; `xAlpha` stays the mirror for balances), Timelock-settable per the build-phase doctrine, and **redeploy** (the oracle is a clone — cheap, and oracles are designed replaceable, [[oracle-replaceable-timelock-wiring]]). The 8x-01 "swap in mirror later with no surface change" discharge is correct for 8-B5/8-B6/8-Bx (ERC20 balance only) but NOT for this consumer. | 8x-02 (2026-06-09, superintendent review) | OPEN |
...
| ~~**Engine (8-B9 / 8-B13)** · exit-numeraire conversion handoff~~ | **VOIDED 2026-06-09 — same dead mechanism as the auto-dump.** This row presumed the windowed in-kind ragequit is the working exit path and fretted about its basket-mix degradation. But `processWindow` reaches only the free main Safe, and the freeze commits `utilization%` of the basket to the non-RQ sidecar → at steady-state high utilization (~80%) a ragequit pays only the unfrozen ~20% (an 80% haircut); full NAV is reachable only at wind-down when utilization/freeze collapse. So windowed RQ is **wind-down-only**, not a steady-state exit — the impatient exit is **CoW**. The basket-liquidity-management concern doesn't apply. (Superseded by the CoW-exit spec being authored.) | Exit Gate (JC1, 2026-06-08) | **VOIDED** |
...
| **Item 10 / deploy + 8-B11/CRE** · warehouse seal + drain-defense (8-Bw, security HIGH) | (a) `setExpectedWorkflowId(...)`→`transferOwnership(timelock)` with `getExpectedWorkflowId()!=0` asserted before the hand-off (§17 build-phase: ownership transfers to the Timelock, NOT renounced) — **do NOT fund the warehouse before the identity is set** (while `expectedWorkflowId==0` the per-workflow gate is OFF, any Forwarder-relayed workflow can drive ops); a **distinct Forwarder identity/workflowId** from the controller/registry/oracle. (b) wire `repaySink = ZipRedemptionQueue` (item 9) — it MUST be **immutable/non-sweepable** (the residual chokepoint; a compromised CRE can REDEEM→REPAY to it). (c) **ELEVATED from optional → recommended:** a Roles `WithinAllowance` rate-limit and/or a **Delay Modifier** on the drain-capable REDEEM/REPAY ops. (d) the CRE issues **exact-amount APPROVE** per SUPPLY (never standing-infinite); verify whether the live EulerEarn is upgradeable / curator-controlled. (e) GOD-EOA→multisig owner upgrade on BOTH the Safe and the Roles owner. (f) assert `repaySink != juniorBaalSafe` and `safe != juniorBaalSafe` (§11 non-commingling). | 8-Bw (2026-06-09) | OPEN |
| **Item 9 · `ZipRedemptionQueue`** · warehouse REDEEM/REPAY seam | `settleEpoch` funds via the warehouse **REDEEM** (USDC → Safe, `receiver==owner==Safe`) then a **REPAY** to the queue (`repaySink == queue`); the queue never calls EulerEarn directly. The queue is the `repaySink` (must be immutable/non-sweepable, above). | 8-Bw (2026-06-09) | **DISCHARGED (by item 9, 2026-06-09)** — the queue references/calls **no** EulerEarn (verified: `grep -i eulerearn` on the source matches only a doc comment; no `IEulerEarn` import), settles against its **own** USDC balance (`balanceOf(this) - reservedAssets`), and is **non-sweepable** (no owner/sweep/pause; `settleEpoch` moves no USDC out; KR-2 no-sweep-selector ABI negative test). Fork-proven: real `WarehouseAdminModule` REPAY → queue balance rises by exactly `amount` → `settleEpoch` fills pro-rata at par; real `ESynth.burn` drops `totalSupply` (own-balance burn, no allowance/capacity). |
| **Item 10** · deploy/wiring (`LienXAlphaEscrow`, 8-Bx) | wire the 4 immutables: `xAlpha`→the bridged xALPHA (8x-01 `SzAlphaMirror`; stand-in until the lane is live); `coordinator`→the loss-side orchestrator that posts launch bonds (and, once built, the M2 `DefaultCoordinator`); `capitalSink`→the off-chain recovery/bridge account (alpha→TAO→USDC); `sidecar`→the 8-B1 substrate **sidecar Safe**. **Assert** all four wired correctly + the coordinator has **approved** the escrow over the protocol's launch xALPHA before any `lockXAlpha` + the wired `xAlpha` is **non-fee-on-transfer / non-rebasing** (load-bearing — the contract has no balance-delta defense; a generic mock is fine in tests, the production token swap MUST be asserted hookless+feeless). **Operational invariant:** the coordinator MUST terminalize every locked lien (release, or slash-then-cohort) — a never-resolved bond is permanently stuck (non-sweepable by design); do NOT lock a bond against a coordinator that may be decommissioned without a live successor. | 8-Bx (2026-06-09) | OPEN |
| **`DefaultCoordinator` (M1-scope, §4.6/§11)** · slash split-policy + driver (8-Bx) | the coordinator is `LienXAlphaEscrow.coordinator` for the slash path — it computes the **capital-vs-premium split** off-chain (the shortfall after foreclosure + insurance) and calls `slashXAlphaToCapital(part)` then `slashXAlphaToCohort()`; on a clean recovery it calls `releaseXAlpha`. The escrow enforces ONLY the routing + `amount ≤ bond` (the dumb-router boundary, §4.6); the split + timing + default-state policy live in the coordinator (the §13 trust boundary — the escrow guarantees destination integrity, not authorization correctness). | 8-Bx (2026-06-09) | **DISCHARGED (by DefaultCoordinator, 2026-06-09)** — the coordinator IS `LienXAlphaEscrow.coordinator` (set-once `setEscrow` + `forceApprove(max)`); RESOLVE/WRITEOFF call `slashXAlphaToCapital(capitalSlashAmount)` if `>0` then `slashXAlphaToCohort()` (reading `escrow.bondAmount` to skip cohort when a full-bond capital slash cleared it), clean RELEASE calls `releaseXAlpha`; the split (`capitalSlashAmount`) is the off-chain CRE arg, the escrow enforces only `amount ≤ bond`. Integration-fork-proven against the real escrow + the two `msg.sender` gates. |
| **`SzipNavOracle` / §12** · sidecar xALPHA valuation (8-Bx) | the sidecar Safe's xALPHA balance — grown by `slashXAlphaToCohort` (the in-kind premium) — MUST be in the NAV basket read (the oracle already reads sidecar balances, §7) so the premium accretes the frozen cohort's `navPerShare` pari-passu — this is what makes route-to-sidecar the socialized pro-rata (the §4.6 fix this window). Confirm the xALPHA leg of the sidecar is valued; no new oracle work expected. | 8-Bx (2026-06-09) | OPEN |
| **Item 10 · `ZipRedemptionQueue` wiring** (created by item 9, security F6/F7) | (a) wire `WarehouseAdminModule.repaySink == ZipRedemptionQueue` (Roles scope pins `EqualTo`); (b) deploy the queue with `controller =` the CRE redemption-settle operator (a **distinct** identity); assert `controller != 0` + non-sweepable before going live; (c) **the REPAY Roles-scope re-target authority (who can change `repaySink`) MUST be the timelock/multisig, NOT the CRE settle operator** (a re-scope can redirect senior redemption funding); (d) the senior-queue `audit/2` L-rows + `audit/3` authority rows (deferred here like 8-Bw, not authorable against an un-wired system). NatSpec already documents that a 7540 `operator` has full claim control incl. arbitrary `receiver` (F7, intended). | item 9 (2026-06-09) | OPEN |
...
| **`ZipRedemptionQueue` (item 9)** · `Withdraw` event lacks `requestId` (GRAPH-01 B-1) | `ZipRedemptionQueue.Withdraw(sender,receiver,controller,assets,shares)` (`:142`) carries **no `requestId`**, so the subgraph cannot deterministically close a *specific* `RedemptionRequest` when an owner has multiple pending (it FIFO-matches as the M1 workaround). **Add `uint256 indexed requestId` to `Withdraw`** (already tracked internally for `RedeemRequest`). LOW severity — aggregate claimed totals are exact; only per-request attribution is approximate. Subgraph-driven ask, no rush; fold into any future `ZipRedemptionQueue` touch. | GRAPH-01 (2026-06-09) | OPEN |
| **Item 10 / subgraph deploy** · addresses + startBlocks + the external peg-pool datasource | the subgraph `subgraph.yaml` `source.address`/`startBlock` for all 14 fixed datasources are `<ITEM-10>` placeholders (GRAPH-01 §7); ALSO the GRAPH-02 zipUSD-**peg** history needs a NEW external **Hydrex/Algebra pool** datasource (`Swap`/`globalState` → `PegSnapshot`) whose pool address is a deploy/liquidity-bootstrap output (DEC-03). Wire all addresses + the peg-pool datasource + run `graph codegen`/`build`/`test` (graph CLI not installed now → deferred acceptance). | GRAPH-01/02 (2026-06-09) | OPEN |
| **Item 10 · 8x bridge deploy/wire** (created by 8x-01) | (a) supply the real `NETUID` + `VALIDATOR_HOTKEY` at the 964 `SzAlpha` deploy (fixtures until subnet/validator registration); (b) run `DeploySzAlphaBridge.deploy964` on a **964 RPC** to exercise the 964 CCT 5-address asserts (un-fork-testable here — no public Subtensor fork node); (c) once BOTH pools exist, call `setRemoteLane` per direction with ops-decided rate limits, under the timelock; (d) transfer the **pool** ownership to the timelock (2-step `Ownable2Step`) AFTER `applyChainUpdates`, and set the token `ccipAdmin` → timelock/multisig (the `deployBase` path already hands the mirror's `DEFAULT_ADMIN`+`ccipAdmin` to the timelock + revokes the deployer); (e) **calibrate the TAO/alpha/rao denomination** of `deposit`'s `msg.value==amount` against the live 964 runtime (the wrapper is delta-robust via S4 but the unit convention is deploy-time); (f) wire the deployed `SzAlpha`/mirror as the `xALPHA` leg into 8-B5/8-B6 (basket LP) + 8-Bx `LienXAlphaEscrow.xAlpha` (replacing the stand-in). | 8x-01 (2026-06-09) | OPEN |
| **`8x-02` xALPHA-APR CRE** · rate source (created by 8x-01) | 8x-02 reads `SzAlpha.exchangeRate()` (the exact `IXAlphaRate` face) to annualize the intrinsic APR; it builds against the M1 stand-in until the lane is live, then repoints to the deployed `SzAlpha`. No new surface owed by 8x-01 (the face is final). | 8x-01 (2026-06-09) | DISCHARGED-INTERFACE |
| **Item 10 / S2 wiring** · `RecycleModule.divert` slots + the one-bank assert (created by S1) | (a) wire `setNavOracle(SzipNavOracle)` / `setEePool(EE_POOL)` / `setWarehouse(CreditWarehouse Safe)` on the deployed `RecycleModule`; (b) **deploy-time assert `RecycleModule.warehouse ==` the `ZipDepositModule`/`WarehouseAdminModule` warehouse Safe AND `RecycleModule.eePool == ZipDepositModule.eePool()`** — one bank; else diverted USDC supplies the wrong pool and never fills the hole (revert the deploy on mismatch). Stream 1's `capitalSink` USDC output must also be supplied to THIS same warehouse (a CRE/off-chain step, not on-chain). `RecycleModule.navOracle` must be the SAME `SzipNavOracle` the `DefaultCoordinator` writes `provision` to (the divert bound reads the live hole). | S1 (2026-06-09) | OPEN |
| **Item 10 / loss-side audit sweep** · `divert` L-step + invariant-fuzz (created by S1) | author the divert into a new `audit/2.md` loss-phase step (post → default → provision → `divert` → warehouse backing up; N-steps `NoHole` / `ExceedsHole` (±1) / over-ledger (`InsufficientFreeValue`) / non-operator / `NoSharesMinted` / `BackingShortfall` each revert) + an **invariant-fuzz** (`divert` never sends more than `min(freeValueAccrued, provision()/1e12)`) + the matching `audit/3-results.md` authority rows (operator-only; warehouse-pinned receiver; no NAV/provision write). Author once the loss side is integration-testable with item-10 (deferred like the 8-B5..B10 sweeps). | S1 (2026-06-09) | OPEN |
| **8-B11 / CRE §8** · divert sizing + the Recovery follow-up (created by S1) | the CRE sizes the per-call `usdcAmount` within `min(freeValueAccrued, provision()/1e12)`, sequences it against the recycle spend, and — after a divert fills the bank — writes a `DefaultCoordinator.Recovery` to reduce `provision` by the realized fill (the divert does NOT write provision itself). | S1 (2026-06-09) | OPEN |
...
user decision.
- **8x-02 R-1 — CAN CRE READ 964? (CONFIG-VERIFY, downgraded from BLOCKING after reading `SzAlpha`, 2026-06-09.)**
  I first overstated this as "`exchangeRate()` staticcalls the `0x805` precompile so it may not work." WRONG —
  `SzAlpha.exchangeRate()` (`SzAlpha.sol:201`) does NOT make a typed precompile call; it uses the **workaround** —
  low-level `STAKING_V2.staticcall(abi.encodeWithSelector(getStake...))` (`SzAlpha.sol:308-309`) — the pattern that
  DOES reach the runtime (the "8x exception" `reference/evm-bittensor/solidity/stakeV2.sol:46` is about *typed*
  calls, which `SzAlpha` avoids). So `eth_call` to `exchangeRate()` works. **Residual = config:** confirm CRE
  supports a 964 chain selector + a 964 RPC, and a test `eth_call` to live `SzAlpha.exchangeRate()` returns. Verify
  at CRE-03; the read pattern itself is proven. (Ticket 8x-02 R-1.)
- **8x-02 R-2 — `SzipNavOracle` rate read has NO freshness gate (HIGH, fund-safety, OPEN, 2026-06-09).**
  `SzipNavOracle._xAlphaUSD()` reads `IXAlphaRate(xAlpha).exchangeRate()` with no staleness check (trusts the M1
  stand-in). Wiring it to the PUSHED `SzAlphaRateOracle` means a stale/zero rate silently mis-marks the xALPHA NAV
  leg, which gates issuance/exit. **Fix (its own focused change — touches the 42-pinned `SzipNavOracle` suite, NOT a
  silent item-10 line): split the rate source (consume `SzAlphaRateOracle`; token stays the mirror for `balanceOf`)
  + gate the rate on `fresh()` (revert issuance / fall back to TWAP on stale).** This is the real "resolves §8.6
  seam" completion. (Ticket 8x-02 R-2.)
- **Oracle wiring = Timelock-re-pointable, NOT set-once-renounce-frozen (USER-CONFIRMED 2026-06-09; memory
  [[oracle-replaceable-timelock-wiring]]).** The branch `SzipNavOracle.sol` setters were intentionally changed from
  set-once (`AlreadyWired`) to Timelock-re-pointable so oracles can be deployed-multiple / hot-fixed. **RECONCILE BEFORE
  ITEM-10:** (a) the `DefaultCoordinator` ticket's "set-once `defaultCoordinator`, immutable after renounce" security
  thesis + NatSpec + the item-10 renounce gate must restate to "sole writer, Timelock can re-point" (still safe —
  Timelock = governance); (b) oracle-consumers holding a **set-once `navOracle`** address (`DurationFreezeModule`, the
  Exit Gate, the zap) need a re-point path OR a re-clone/re-deploy on an oracle swap (cheap for the clones) — item-10
  wiring. Not a contract bug; an authority-model reconciliation across the DC + item-10 + consumers.
...
- **8-B10 recycle/payout (2026-06-08) — 2 §4.5.1 8-B10 consistency spec-gaps FIXED in `claude-zipcode.md`, no §17 reopened.**
  ...
  (Loot is soulbound + gate-held; the transferable share is szipUSD). `auto-compounder.md §6` says "pro-rata to szipUSD
...
- **Exit Gate + szipUSD authoring (2026-06-08) — 2 spec gaps FIXED in `claude-zipcode.md`, no §17 reopened.** Both
  ...
  in-kind; zipUSD→USDC is `ZipRedemptionQueue`.)* *(The xALPHA→zipUSD auto-dump module was REMOVED 2026-06-09 — the impatient exit relies on CoW, not an auto-dump.)*
...
- **SzipNavOracle ZERO-GUESS GATE CLOSED (2026-06-08) — independently re-materialized from the ticket alone, 3
  ...
  idempotence test — both builders had to deviate identically). (Gate report folded into the ticket + deleted.) The
  kept tree is green; the gate is now formally met for `SzipNavOracle`.
...
- **Supply-side redesign — RESOLVED 2026-06-05 (user-directed + ratified; folded into `claude-zipcode.md`
  ...
  5. **M1 cut:** items 7 (zap → `USDC→zipUSD→szipUSD vault`) + 8 (szipUSD = the M1 vault SHELL) ship the simple
     end-state shape; the compounder/xALPHA-buyback/POL bolt on post-M1. **The zap (item 7) is UNAFFECTED by the
...
- **2026-06-10** (**item 10 — deploy/wiring orchestrator `DeployZipcode.s.sol`; AUTHORED + `forge build` GREEN**) — Authored the item-10 deploy script FROM `wires/` (the wiring map is the spec). Filed `tickets/woof/WOOF-10-deploy-wiring.md` (the phase-by-phase P0–P9 deploy order + every verified ctor/`setUp`/sub-deployer entrypoint + the Do-NOTs + the inline seam asserts), then cold-built `contracts/script/DeployZipcode.s.sol` via a focused subagent (zero load-bearing guesses — every signature read against the kept code; the subagent ran its own single `forge build`, no concurrent builds). **Result: `forge build` GREEN** (625 lines, `contract DeployZipcode is SummonSubstrate`, `_phaseP0..P9`, entrypoint `deploy()`). Deploys the full Base-side protocol in dependency order, composing the three existing sub-deployers (`SummonSubstrate._summon` inherited, `CreditWarehouseDeployer.deploy`, `ReservoirMarketDeployer.deploy`) + 13 standalone ctors + the rate oracle + 9 engine-module clones (ModuleProxyFactory `deployModule` + `setUp` + a `_enableModuleOnSafe` Safe-`execTransaction` helper modeled on `SummonSubstrate._addOwnerToSidecar`; DurationFreeze enabled on BOTH Safes) + the loss side + the four NAV-oracle wiring setters + the rate seam. **Circular ctor deps resolved by the build-phase Timelock-settable setters** (the wires/ #1 finding — adapter↔controller, hook↔borrowDriver, escrow↔coordinator all deploy with a placeholder then `setX`). **All 8 cross-cutting seams asserted inline** with named errors (one-bank trio / shared-LP / engineSafe trio / venue-registry / zero-Shares / gate-shareToken / escrow↔coord / non-commingling). **Identity seal:** `setExpectedAuthor`/`setExpectedWorkflowId` on every ReceiverTemplate → `ZipcodeDeployAsserts.requireIdentityWired` pre-gate → `transferOwnership(timelock)` everywhere (NOT renounce, §17 build phase). **5 build-discovered code-vs-spec deltas folded into the ticket:** (1) `ZipDepositModule` has no Ownable (immutable `deployer` + re-settable `setGate`) → no transfer; (2) `ZipRedemptionQueue` deployed in P4 with `zipUSD=address(0)` then `setTokens` in P3 (warehouse needs `repaySink==queue` first); (3) entrypoint `deploy()` not `run()` (SummonSubstrate.run collision); (4) EE-pool admin config (createEulerEarn/allocator/curator/fee + supply-queue→reservoir) is **fork-only** — the local `IEulerEarn` shim avoids the 0.8.26 source, so `EE_POOL`/`BASE_USDC_MARKET` are env inputs and that wiring is a documented runbook TODO (obligations 311/333 still owed, scoped to the fork-execution step); (5) all else matched code exactly. NOT git-committed. Memory [[wires-map-item10-foundation]]. `reports/WOOF-10-report.md`. **Runnable skeleton teed up for the next window:** `contracts/test/DeployZipcode.t.sol` (fork harness — `setUp` forks Base + sets all env, 3 `vm.skip(true)` acceptance stubs [Phase-S post-state / identity-pregate tested-negative / L4 origination] with the stand-in TODO inventory + the intended seam assertions; `forge build` GREEN) + `contracts/.env.example` extended with every `_loadInputs()` key (principals / `(T)` stand-ins / numeric knobs). **NEXT = item-10 fork-execution verification** — turn the 3 skipped tests green: wire the `(T)` stand-ins (create EE_POOL off the live factory + the EE allocator/curator/fee + supply-queue→reservoir config [obligations 311/333, fork-only]; XALPHA_MIRROR/POL_ICHI_VAULT/POL_GAUGE/IRM/BASE_USDC_MARKET mocks), drive as `team` (broadcaster — maybe add a test-only `_deployWith(Inputs)` + a `getDeployment()` view), then the deferred `audit/2`+`audit/3` engine/junior/loss L-row sweeps, and the 8x bridge two-chain (964) wire. **Superintendent to release.**
- **2026-06-09** (**item-10 phase 0 — `wires/` protocol wiring map; AUTHORED**) — User redirected item-10 away from a blind deploy-script build to a **documentation-first** phase: produce a per-component **wiring map** read from the kept code under `contracts/` as the authoritative final form (tickets/reports/spec = intent only; code wins). Authored `wires/` — **26 component docs** (WOOF-00/01 hand-authored as the template, the other 24 via the harness subagent pattern in cluster waves: venue spine / supply substrate / engine modules / loss / bridge) + **11 interface-folder catalogs** (`interfaces-<algebra|baal|bridge|cow|euler|hydrex|ichi|loss|safe|supply|zodiac>.md`, every `src/interfaces/**` shim cataloged file-by-file with its live Base address / internal-seam classification + consumers) + a synthesizing `wires/README.md` index + **`wires/COVERAGE.md`** (the file-level manifest mapping EVERY `.sol` under `contracts/` — 32 product contracts + 5 scripts + 30 interface shims + 28 tests — to its doc; mechanically verified 0 files missing). Each doc records Role · Contracts involved · Wiring internal (ctor/setUp, immutables, setter-wired pointers, authority/gating) · Wiring cross-component (who points at whom) · **Item-10 deploy facts** (ctor arg order, wiring setters, ownership posture, asserts) · Gotchas. **This is the substrate the item-10 deploy/wiring script is authored from** (connect documented pins, don't rediscover). **The pass surfaced a repo-wide reconciliation** confirmed against the code: many slots the tickets/spec/reports call "immutable / set-once `AlreadyWired` / renounce-frozen" are actually `onlyOwner` Timelock-settable storage in the kept code (registry, controller, warehouse adapter, redemption queue, `LienXAlphaEscrow`, `DefaultCoordinator`, NAV oracle, every engine module) ⇒ deploy posture = `transferOwnership(timelock)` NOT renounce (memory [[oracle-replaceable-timelock-wiring]]; immutability deferred to pre-prod). Several ctors also drifted from the tickets (`EulerVenueAdapter` 10-arg not 9; `SzipNavOracle` 11-arg + a 5th `setXAlphaRateOracle`; `ExitGate` derives loot/mainSafe from `IBaal`; warehouse fields mutable) — `wires/` is now authoritative over the prose for those. `wires/README.md` also captures the 8 cross-cutting seams (ReceiverTemplate identity-seal family · engineSafe denominator-exclusion chain · shared-LP-address invariant · one-bank invariant · repaySink chain · engine-module deploy template · xALPHA rate/balance split) + the deploy dependency order. NOT git-committed (consistent with recent windows). Memory [[wires-map-item10-foundation]]. **NEXT = item-10 deploy/wiring script, authored FROM `wires/` (or a slice of it) — superintendent to pick the first slice.**
- **2026-06-09** (**S1 — loss-side Stream 2 `RecycleModule.divert`; BUILT-VERIFIED + KEPT**) — Driven by `kickoff.md`/`solvency.md` (the loss-side build-spec): the waterfall's one genuine build gap is **Stream 2** (yield diversion → fill the warehouse hole). Added a **`divert` mode to the BUILT `RecycleModule`** (8-B10) — `divert(usdcAmount)` supplies engine free-value USDC straight into the credit warehouse (`eePool.deposit(usdcAmount, warehouse)`, **NO zipUSD minted, NO senior claim**), bounded by the live `SzipNavOracle.provision()` hole; a SECOND spend of the same `freeValueAccrued` ledger `recycle` governs (both debit it, each leaves the other the remainder). **Order load-bearing — bounds-before-spend, then CEI:** ZeroAmount → NoHole → ExceedsHole (`usdcAmount*1e12 > provision()`, strict `>`, scales USDC 6-dp→USD 18-dp) → `_spendFreeValue` (effects first) → approve/deposit/reset execs → value guards → `Filled`. 3 new set-once wiring slots (`navOracle`/`eePool`/`warehouse`, clone-safe non-`immutable`, setUp 5→8, 3 Timelock `WiringSet` setters); 2 local interfaces (`ISzipNavProvision`, `IEulerEarn`); errors `NoHole`/`ExceedsHole`/`NoSharesMinted`/`BackingShortfall`. **Harness: ticket → 5-critic fanout (junior/spec-fidelity/ref-verifier/qa/security) → synthesize/triage → cold-build + KEEP.** **spec-fidelity FAITHFUL — NO spec gap, NO `claude-zipcode.md` edit** (1e12 direction/strictness correct; §17 honored; correctly does NOT route to capitalSink and does NOT write provision — the CRE reduces the hole later via `DefaultCoordinator.Recovery`). **Security found 2 MEDs:** F2 huge-provision→drain-free-value = §13-accepted **grief** (USDC still lands in the bank; bounded by trusted Coordinator+CRE + the deferred invariant-fuzz), NOT theft; **F5 — the spec's required share-rose guard proves *liveness* not *value-moved*** (a malicious EE could mint warehouse shares without pulling USDC) → **promoted the USDC-fell hard-backing assert INTO the contract** (`BackingShortfall`: Safe USDC fell by EXACTLY `usdcAmount`), matching the module's own HARD-BACKING doctrine — preserve-intent hardening, flagged for superintendent (demotable to test-only if you want spec-minimal). qa: the value-flow test MUST use a LIVE Safe + real `EEMock` (a non-live RecordingSafe falsely trips the share-rose guard); folded the ±1 boundary + two-bounds-bind + overflow-panic + stingy/free-mint/readback mocks. **15 new tests** — 13 `RecycleModuleDivertTest` (value-flow+exec-shape, ±1 boundary, two-bounds-bind, overflow-panic, BackingShortfall, NoSharesMinted, bubble/ExecFailed atomicity, CEI readback decrement-before-exec, divert+recycle co-existence, NoHole/ZeroAmount/NotOperator) + 1 Base-fork divert vs a real summoned Safe + 1 Stream-2 setter test — **ZERO load-bearing guesses; all prior recycle tests stay green unchanged.** Code `contracts/src/supply/szipUSD/RecycleModule.sol` (the `divert` mode) + `contracts/test/RecycleModule.t.sol`; `forge test --match-path test/RecycleModule.t.sol` 32/32 non-fork + 2/2 fork. NOT git-committed (consistent with recent windows). **Obligations CREATED:** item-10/S2 wiring (`setNavOracle`/`setEePool`/`setWarehouse` + the one-bank deploy-asserts `warehouse == ZipDepositModule's warehouse` AND `eePool == ZipDepositModule.eePool()`); the loss-side `audit/2`+`audit/3` sweep + the `divert ≤ min(freeValueAccrued, provision/1e12)` invariant-fuzz; 8-B11 CRE divert-sizing + the Recovery follow-up. Updated `solvency.md` status (Stream 2 BUILT). `tickets/sodo/S1-recycle-divert.md`; `reports/S1-report.md`. **The loss side is now fully built on-chain. NEXT = S2 item-10 deploy/wiring (not a contract).**
- **2026-06-09** (**8x-02 — xALPHA-APR CRE feed; on-chain receiver BUILT-VERIFIED + KEPT, Go workflow authored**) — Superintendent picked 8x-02 (over item-10 / Exit auto-dump) via AskUserQuestion. The ticket was already authored/build-ready, so this window ran a focused **4-critic fanout** (junior/spec-fidelity/ref-verifier/security) on the on-chain `XAlphaAprOracle` receiver, then cold-built it. **Critics converged hard; 7 binding resolutions (R1–R7) folded into the ticket pre-build** (no re-fan — all convergent clarifications/hardening, README §3a): R1 payload = a FLAT 8-field tuple `(uint32 intrinsicAprBps, uint256 exchangeRate, uint32 windowSeconds, uint32 forwardAprBps, uint256 xalphaPriceUsd, uint32 incentiveAprBps, uint48 computedAt, uint32 flags)` (NOT `SzipNavOracle`'s batched-leg shape — junior's #1 most-blocking catch); R2 guard order (FutureTimestamp→ZeroRate→**AprCapExceeded UNCONDITIONAL incl. cold first-push**→deviation-only-if-prior), single atomic struct write; R3 deviation = **one-sided UP absolute bps band** (down moves always publish — yield decline OR slash legitimate; resolves the security-CRITICAL brick-on-slash contradiction without a div-by-near-zero relative band); R4 **monotone-rate sanity = WORKFLOW guard, NOT receiver** (receiver holds no prior rate, `uint32` can't be negative → a slash publishes `0`+flag, lands); R5 owner=Timelock (build-phase doctrine [[oracle-replaceable-timelock-wiring]], NOT renounced — the security critic's "renounce at deploy" cite was STALE post the 2026-06-09 repo-wide conversion; `SzipReservoirLpOracle:18` comment is likewise stale) + the `setForwarderAddress(0)` footgun documented; R6 ctor `(forwarder, maxDeviationBps_, aprCap_, maxAge_)` with `aprCap_<=type(uint32).max`, NO `IXAlphaRate` on-chain read (caches pushed rate); R7 reads all `view`, advisory, never fund-gating. **Spec edit (the one spec gap):** §8.0 envelope table gains the `8` APR / `XAlphaAprOracle` row + the keying note — `8` is a non-collision with `DefaultCoordinator`'s `8` (keyed by `(receiver,reportType)`); the ticket's earlier "don't reuse 7 / next free" framing wrongly implied a global type space, corrected. **Cold-build (fresh subagent, from the ticket alone), ZERO load-bearing guesses, independently re-ran green: 20/20 receiver suite** (incl. `test_firstPushBoundedByCap`/`test_oneSidedDeviation`/`test_legitimateSlashFlagsNotBricks`/`test_deviationRevertPreservesLastGood`/ctor `InvalidAprCap@uint32`/owner-repoint-vs-`OwnableUnauthorizedAccount`), **678/678 total non-fork no regression** (`forge test`). Code `contracts/src/bridge/XAlphaAprOracle.sol` + `contracts/test/bridge/XAlphaAprOracle.t.sol`. **Go workflow authored as the CRE-03 integration artifact** `cre/xalpha-apr/{main.go,README.md}` (the FIRST CRE workflow in the repo — establishes the `cre/` location): the **load-bearing contract-coupled parts pinned exact** (the payload ABI tuple matching the on-chain decode; the §primary `(rNow/rPrev−1)×(year/Δ)` formula, all-integer/deterministic, floored-at-0; stateless-prior-via-receiver-cache design), with `go.mod`/RPC-config/the §secondary metagraph `0x802` + §value price legs marked `TODO(CRE-03)` — NOT compiled in the Foundry repo (no Go toolchain there; honestly flagged, mirrors 8x-01's integration-time lane flag). NOT git-committed (superintendent decision, consistent with recent windows). **Obligations CREATED (logged):** item-10 wires `XAlphaAprOracle` (deploy under Timelock, set knobs, `transferOwnership(timelock)`, pre-prod renounce decision) + 8-B12/8-B11 consume `intrinsicAprBps`/`flags`; CRE-03 compiles+deploys the Go workflow (+ resolves the cross-chain supply seam for the value leg jointly with the §8.6 `NAV_LEG` leg-0 producer — ticket safety §8). `tickets/bridge/8x-02-xalpha-apr-cre.md`; `reports/8x-02-report.md`. **NEXT = item 10 (deploy/wiring) / the CRE off-chain track / Exit auto-dump — superintendent to pick.** **POST-CONCLUSION REDESIGN (user-directed, same day) — DERIVE, DON'T PUSH.** The superintendent called out that an APR (and NAV) is `f(exchangeRate, time)` — both on-chain reads — so a Forwarder-gated push-cache receiving a pre-computed APR + adversarial deviation bands was over-built and backwards (machinery to transport + defend a number the chain derives itself; the band was self-inflicted — it spawned the "slash bricks the feed" knot it then "solved"). **Collapsed `XAlphaAprOracle` to an ON-CHAIN DERIVATION:** ownerless/immutable/Forwarder-less, reads `IXAlphaRate.exchangeRate()` natively on **Subtensor 964** (the Bittensor constraint — the rate is only native there; the Base mirror is a plain BurnMintERC20), two rolling checkpoints via a permissionless `poke()`, derives `(rNow/rPrev−1)×year/Δ` on read (floored 0 = slash-safe, clamped to an immutable display `aprCap`). Off-chain consumers (UI/8-B12/8-B11) RPC-read 964 directly; advisory ⇒ never bridged to Base. **DELETED:** the receiver/Forwarder, `APR=8` report type (+ its §8.0 row, reverted), the pushed payload, the deviation band + fallout, and the whole Go workflow (`cre/xalpha-apr/`). **15/15 derivation suite, 673/673 total non-fork no regression** (was 678 with the 20-test push version). §8.8 rewritten to the derivation model. The lesson: derive where the data lives, push only the irreducible cross-chain primitive (here the rate — and even that only IF a Base contract ever needs it; none do). **v3 FINAL (user-corrected the v2 placement — a Base contract DOES need it):** the protocol + the NAV/Euler oracle live on Base, and `exchangeRate` is native only on 964, so the cross-chain pull IS required and it IS a CRE job. The fix vs v1 wasn't "no CRE" — it's **CRE pulls the RATE (the primitive), not a pre-computed APR.** Shipped `SzAlphaRateOracle` on Base (`is ReceiverTemplate, IXAlphaRate`): a CRE workflow (`cre/szalpha-rate/`) pulls `SzAlpha.exchangeRate()` from 964 and pushes it RAW (`reportType RATE=8`, `(uint256 rate, uint48 ts)`); the contract exposes `exchangeRate()`/`fresh()`/`lastUpdate()` (the Base-side `IXAlphaRate` drop-in for `SzipNavOracle` + Euler adapter) and a DERIVED `intrinsicAprBps()` view. Truthful guards (non-zero/not-future/strictly-newer), NO deviation band (a slash legitimately lowers the rate), consumers fail-closed on `fresh()`. **Resolves the §8.6 cross-chain rate seam.** DELETED the v1 push-APR + v2 964-native `XAlphaAprOracle`; the Go workflow is now the raw-rate pull. **17/17 suite, 675/675 total non-fork.** §8.8 rewritten; §8.0 gains the `8` RATE / `SzAlphaRateOracle` row. **Item-10:** deploy `SzAlphaRateOracle` on Base + run the pull workflow + point `SzipNavOracle`'s xALPHA RATE read at it (token addr stays the mirror for balances).
- **2026-06-09** (**`DurationFreezeModule` — Duration-Bond trigger B; BUILT-VERIFIED + KEPT**) — Superintendent picked the duration-squeeze freeze (over `8x-01` / item-10) after I clarified what it is: the **rotation/floor actuator** that fills/drains the non-RQ sidecar so the Exit Gate's window exit structurally reaches only free equity. The substrate (two Safes, 8-B1) + the Gate's structural-freeze half are already built; the **orphan** was the actuator (baal-spec §8 parked it as "item 9 / 8-B11", but item 9 became `ZipRedemptionQueue` and 8-B11 is the off-chain CRE robot — so it never got a build slot; the stale `// (item 9 rotation)` comment in `ExitGate.sol:40` is the fingerprint). **User confirmed scope via AskUserQuestion = the FULL module** (autonomous on-chain floor + operator-driven asset rotation, not trigger-state-only / not folded into CRE-05). **Design = the `DefaultCoordinator` "bounds-not-validates" pattern:** a Zodiac `Module` enabled on BOTH Safes — `commit`(main→sidecar, operator-gated, **ungated by value** — increasing the freeze is peg-safe) + `release`(sidecar→main, operator-gated, **autonomously floor-gated**: reverts unless `committedValue() ≥ requiredFraction(U)×grossBasketValue`); `requiredFraction = min(1e18, max(U, escalation))`, `escalation = maxLockFraction×clamp((U−U_lock)/(U_max−U_lock),0,1)` (§11-B); gates the Exit Gate **structurally** (no ExitGate change). **Drafted the build-only ticket → 5-critic fanout** (junior/spec-fidelity/ref-verifier/qa/security). **Spec-fidelity FAITHFUL** on all 5 self-flagged tensions (autonomous floor = faithful §11-B/§6.4 detailing per the §4.6 precedent; `max(U,esc)` with φ_A=utilization correct for the liquidity path; `commit` ungated + `maxLockFraction`=escalation-cap is canonical §11, baal-spec §8.8's "redeemability cap" phrasing is **stale**, dies with that file; dropping `maxDuration`/`releaseHysteresis` faithful under the continuous floor; stays out of the loss side). **Ref-verifier:** every sig/path OK (minor drifts: `setUp` body `:69-97`; `_summon` returns a `Substrate` struct; the Baal `enableModule` idiom substitutes the proven `addOwnerWithThreshold` path; `EulerEarn.asset()` is ERC4626-inherited not declared — all folded). **SECURITY found a CRITICAL + I fixed it FIRST (spec surgery):** my v1 `utilization()` read `idle = IERC20(usdc).balanceOf(eulerEarn)` was **both wrong and exploitable** — EulerEarn is a pure Morpho-style allocator so `totalAssets()` excludes idle (idle≈0 → `U≈1` always → **bricks every release**), AND `balanceOf(eulerEarn)` is **donatable by anyone** (transfer USDC to the pool → inflate idle → lower U → lower floor → over-release that **opens the run hatch**, defeating §11-B "not outsider-manipulable"). Verified against the real `EulerEarn.sol` (`totalAssets` = `Σ expectedSupplyAssets` :903, donation-immune; `maxWithdraw` :546) → **redesigned the read to `U = 1 − maxWithdraw(warehouse)/convertToAssets(balanceOf(warehouse))`** (the illiquid fraction of the senior backing, off the controller-gated borrow side, donation-immune) and **fixed spec §8.2 + §11-B** (the §8.2 "U = borrowed/totalAssets" / "idle" framing was the spec's own error). **Security HIGH:** non-basket-asset leak (`release` of an oracle-unvalued asset drains the sidecar while the floor passes) → added the **5-leg oracle whitelist** read from the oracle at `setUp` (also closes the rebasing/FoT concern). **All critic hardening folded into the ticket (no re-fan — convergent, README §3a):** the additive oracle `committedValue()`/`freeValue()` (grossBasketValue **UNCHANGED** — the 42-test pins hold; parity exact for the 5 plain legs, ≤2-wei for the split LP; the module never moves the LP so gross is exactly rotation-invariant); the `FreezeHandler` 128k-call invariant (`committedValue ≥ requiredCommittedValue` post-release); the `mainSafe!=sidecar` guard; the FoT mock + atomic-rollback (balances+committedValue restored on `FreezeFloorBreach`); the truncation-pin vector; the escalation-bites + exact-equality `U==uMax` vectors; the donation-immunity test; the two-Safe `enableModule` fork plumbing; the `setTarget`-inert note. **Cold-build (fresh subagent, superintendent-released same window after the spec fix), ZERO load-bearing guesses, independently re-ran green from `forge clean`: 48/48 module** (unit + the FreezeHandler 128k-call invariant [256 runs × 500 depth, 0 reverts, `committedValue ≥ requiredCommittedValue` post-release] + 2 Base-fork: real `_summon` substrate + real `SzipNavOracle` cross-Safe rotation + a forced `FreezeFloorBreach`), **633/633 total no regression** (independently re-run from clean — the subagent had hit forge incremental-cache phantom failures that vanished on `forge clean`; the authoritative clean run is green), the **`SzipNavOracle` 42-test suite + `grossBasketValue` exact pins UNCHANGED** (additive-only extension). Code `contracts/src/supply/szipUSD/DurationFreezeModule.sol` + `contracts/src/interfaces/{euler/IEulerEarnUtil,supply/ISzipNavBasket}.sol` + the additive `SzipNavOracle` views + `contracts/test/DurationFreezeModule.t.sol`. NOT git-committed. One build-exposed faithful-clarification: the invariant is asserted as a **per-successful-release ghost check** (not a global function — `commit` is ungated + `U` floats, so a later bump legitimately raises the floor above current committed; the ticket's "after any successful release" is exactly this). **BUILD-SURFACED, NOT MINE — flag to superintendent:** the branch working-tree `SzipNavOracle.sol` had **already** converted its four set-once wiring setters (`AlreadyWired`) to **Timelock-re-pointable** setters (with `SzipNavOracle.t.sol` updated to match) — an authority-model change that touches the item-10 renounce/freeze + the DefaultCoordinator "set-once `defaultCoordinator`" assumption; left intact, surfaced for review. **Spec edits:** `claude-zipcode.md` §8.2 (donation-immune U read) + §11-B (corrected trigger line + the M1 continuous-floor realization note subsuming `maxDuration`/`releaseHysteresis`). **Obligation DISCHARGED-pending-build:** PROGRESS row 289 (sidecar-funding-after-team-owns; structural + item-10 wiring). **Obligations CREATED:** item-10 (deploy/clone/setUp + enable-on-both-Safes + warehouse wiring + the **live-pool U donation-immunity verification** — the CRITICAL's residual: confirm the production EVK strategy-cash donation surface is bounded); CRE-05 operator drives the rotation; governed `uLock/uMax/maxLockFraction`; the audit/2+3 sweep. Ticket `tickets/sodo/DurationFreezeModule.md`; `reports/DurationFreezeModule-report.md`. **NEXT = `8x-01` cold-build / item 10 (deploy/wiring) — superintendent to pick.** (Plus the surfaced `SzipNavOracle` set-once→Timelock setter change to review.) **POST-CONCLUSION SIMPLIFICATION (superintendent-directed, same day): collapsed to DEAD-SIMPLE -- requiredFraction(U) = U (freeze% = utilization%); the as-built U_lock/U_max/maxLockFraction escalation was STRIPPED from the contract (post-M1). Re-verified green: 40/40 module, 628 total. Spec Â§11-B + the ticket updated.**
- **2026-06-09** (**`DefaultCoordinator` — the loss-side orchestrator; BUILT-VERIFIED + KEPT**) — Superintendent picked DefaultCoordinator (over 8x-01 / item-10) for this window; scope confirmed via AskUserQuestion = **full orchestrator, M2 paths mock-tested** (the proven 8-Bx pattern). Authored `tickets/loss/DefaultCoordinator.md` (build-only, no INFLOW) through the full harness. **Derived (not invented) the architecture from the locked seams:** `LienXAlphaEscrow.coordinator` is **immutable** + the sole caller of all four escrow state-changers ⇒ the DefaultCoordinator **must be** that wired coordinator from deploy, owning the **full** bond lifecycle (LOCK/RELEASE M1-live + slash M2); it is **also** the oracle's set-once `defaultCoordinator` (sole `writeProvision` caller). A CRE-gated `ReceiverTemplate`, renounce-frozen, holding the launch xALPHA reserve (`forceApprove(escrow, max)` in the set-once `setEscrow` that breaks the escrow↔coordinator circular deploy). **Spec surgery FIRST (the only spec gaps were the under-defined report ABI + the coordinator's concrete shape):** detailed the §4.6 DefaultCoordinator bullet to build-grade, added the **reportType-8** row to the §4.4 table (action family LOCK/RELEASE/DEFAULT/RECOVERY/RESOLVE/WRITEOFF), and authored §8.4's producer-level action decode + the CRE-01 map rows. **5-critic fanout** (junior/spec-fidelity/ref-verifier/qa/security). **Spec-fidelity verdict FAITHFUL** (the surgery was DETAILING not reopening; both inbound obligations — the provision bound + the slash driver — discharged; no §17 reopen). **Ref-verifier:** every signature/line confirmed against the real `ReceiverTemplate`/`SzipNavOracle`/`LienXAlphaEscrow` (no wrong paths/sigs). **Critic hardening folded into the ticket BEFORE build (all convergent clarifications/test-hardening of already-intended surface — no re-fan, README §3a):** (junior) the missing **`MockNavOracle`** shape for unit tests + the OZ-vs-forge-std `IERC20` trap (forceApprove needs the OZ one) + add `bondOriginator` to the interface + drop the "active set" wording (WrittenOff provision persists in `totalProvision`); (qa) the **stateful invariant handler** with an **independent `ghost_cap`** (the drafted fuzz was tautological — recomputed the formula), the floor boundaries (0 / `1e18-1`-floors-small-atRisk-to-0 / the `atRisk=3,floor=0.5e18→1` truncation pin), RECOVERY status-stays-Defaulted + double-no-op, the **full status×action illegal-transition matrix** (incl. LOCK-on-terminal, which the escrow would allow at `bondAmount==0` — the coordinator's status guard blocks it), WRITEOFF↔RESOLVE parity, the **atomic-rollback** assertion on a bubbled `ExceedsBond` (provision/status unchanged), event co-emission + replay, the 11-arg integration ctor pin, the no-freeze ABI-negative; (security) the **residual-trust NatSpec block** (the §13 boundary stated plainly: bounds+routes, does NOT validate a default is real; atRisk is CRE-supplied so the bound constrains the TRANSFORM not the magnitude; destinations immutable except the CRE-named originator leg; grief-not-theft; NAV never above the un-impaired basket) + the **hard tested renounce gate** (item-10 must `require` escrow/oracle/identity/allowance/two-way-wiring before renouncing — F1/F2/F3: renounce-before-`setEscrow` bricks both contracts, renounce-before-identity opens the workflow-blind grief surface) + the WRITEOFF-must-not-decrement-`totalProvision` Do-NOT (F9). **Cold-build (fresh subagent), ZERO load-bearing guesses, independently re-ran green: 63/63** (57 unit + 3 integration vs the REAL oracle+escrow proving both `msg.sender` gates + 3 stateful invariants over 128k calls/0 reverts), **575/575 total no regression** (`forge test`; independently re-run, deterministic ×2). Two build-exposed test-fixture fixes folded back (the `MockNavOracle` must `emit ProvisionWritten`; assert a non-emit via `vm.recordLogs()`). Code `contracts/src/loss/DefaultCoordinator.sol` + `contracts/src/interfaces/loss/{ISzipNavOracle,ILienXAlphaEscrow}.sol` + `contracts/test/DefaultCoordinator.t.sol`. NOT git-committed (consistent with the recent loss-side/8-B* windows — superintendent commit decision). `reports/DefaultCoordinator-report.md`. **Obligations DISCHARGED:** provision-bound (to `SzipNavOracle`) + slash-driver (to 8-Bx). **Obligations CREATED (logged):** item-10 circular-deploy + the hard renounce gate; the xALPHA just-in-time funding discipline (non-sweepable, feeless token); CRE §8.4 computes atRisk/recoveryProceeds/capitalSlashAmount off-chain; the item-10/M2 audit sweep. **NEXT = `8x-01` cold-build / item 10 (deploy/wiring) / the duration-squeeze freeze — superintendent to pick.**
- **2026-06-09** (**8-Bx — `LienXAlphaEscrow`; BUILT-VERIFIED + KEPT**) — Authored `tickets/loss/8-Bx-lien-xalpha-escrow.md` (per-lien xALPHA first-loss bond custody — the loss-side sibling of item 9) through the full harness: verified the "Model from" against `reference/moneymarket-contracts` directly (`InsuranceFund.bring:33` = the immutable-caller + gated-`safeTransfer` custody model; `MarkdownController.slashJaneProportional:146` = a proportional-slash concept template we REJECT; `jane/RewardsDistributor:131/:141` = a Merkle pull-claim distributor we do NOT build) → drafted (build-only; no INFLOW) → **5-critic fanout** (junior/spec-fidelity/ref-verifier/qa/security). **Spec-fidelity FAITHFUL** (custody/two-job-slash/in-kind-premium/no-share-move all honored; no §17 reopen; confirmed 8-Bx is a leaf — NO inbound cross-ticket obligations owed by it). **One spec-gap caught + fixed FIRST:** §4.6's "snapshot → per-holder xALPHA share / `RewardsDistributor`-style cohort distributor" sentence was **stale M2-sketch language contradicting §11/§6.4's socialized-sidecar model** ("no per-position index, no SBT"; `HaircutLockAccountant`/`RecoveryClaimSBT` not built) → replaced with **route-the-remaining-bond-to-the-sidecar-Safe + NAV-does-the-pro-rata** (spec-fidelity confirmed the contradiction was real; memory `[[prefer-simplest-mechanism]]`). **Critic-mandated hardening folded into the ticket BEFORE build (all convergent, no re-fan — README §3a):** (security #8) **`lienId` key → `bytes32`** to match the canonical controller/factory/venue/CRE-report type (was `uint256`); (security #3) **mandatory `nonReentrant`** on all four state-changers given the contract is token-agnostic (item-10 swaps the production xALPHA in) — keeps CEI as the secondary proof; (security #6) `require(originator != address(this))` (`SelfOriginator`) + the invariant asserts `to != address(this)`; (junior #8) the reentrancy probe was self-contradictory as drafted (all entrypoints are `onlyCoordinator`, so a plain-token callback reverts `NotCoordinator` before reaching the CEI defense) → **fixed by setting `coordinator == the reentrant token`** so the probe actually exercises the guard (`ReentrancyGuardReentrantCall`); (qa) added the pure-premium path, exact-equality boundary, multi-lien independence, re-lock reusability, failed-pull-no-orphan, coordinator-approval/balance negatives, `expectEmit` mandates, the no-sweep ABI-negative, and reframed the fund-flow invariant to mechanically-checkable **balance-conservation** (a stateful `invariant_` handler). **Documented the security residual** (the thesis proves DESTINATION integrity — funds only ever reach {recorded originator, immutable capitalSink, immutable sidecar}, no recipient param ⇒ no theft even from a compromised coordinator — but NOT timing/authorization correctness, which is the coordinator's §13 trust boundary; M1 surface is lock/release only, slash goes live with the M2 CRE-Forwarder-gated `DefaultCoordinator`; no split role needed for M1). **Cold-build (fresh subagent), ZERO load-bearing guesses, independently re-ran green: 44/44** (unit + reentrancy probe + 2 stateful invariants over 128k calls [0 reverts] + fuzz), **512/512 total no regression** (`forge test`; full suite incl. fork — independently re-run). No external interface sigs / no hardcoded addresses (pure custody over a generic ERC-20) ⇒ nothing to chain-verify; no fork test (the bridged xALPHA is not yet live — 8x-01 stand-in). Code `contracts/src/loss/LienXAlphaEscrow.sol` + `contracts/test/LienXAlphaEscrow.t.sol`. NOT git-committed (consistent with item 9 / 8-B* windows — superintendent commit decision). `reports/8-Bx-report.md`. **New obligations 8-Bx creates (logged in the table):** item-10 wiring (+ feeless-token + coordinator-approval asserts + the terminal-call operational invariant); `DefaultCoordinator` owns the slash split-policy; `SzipNavOracle` must value the sidecar xALPHA leg. **NEXT = `8x-01` cold-build / item 10 / `DefaultCoordinator` — superintendent to pick.**
- **2026-06-09** (**DEC-01/02/03 resolutions + `8x-01` szALPHA bridge ticket AUTHORED + 2-round critic-hardened; BUILD-READY**) — User-driven decisions logged + propagated across PROGRESS/PHASE2/`bridge/xalpha-bridge-impl.md`/memory: **DEC-03** = Hydrex LP pair is **xALPHA/zipUSD** (stable leg; confirms the spec's existing `zipUSD/xALPHA` pool — only the "xALPHA/szipUSD closed loop" memory phrasing was stale). **DEC-02** = FORK (`szALPHA` on our own validator) + **CCT registration on chain 964 confirmed OPEN/self-serve** (researched the live CCIP Directory: full TokenAdminRegistry 1.5.0 + RegistryModuleOwnerCustom 1.6.0 + TokenPoolFactory 1.5.1 stack deployed on 964; GTV's own xSN* xAlpha tokens already registered CCTs with a live bittensor→base lane; all 10 router/registry/RMN addresses for 964+8453 captured into the ticket). Then **authored `tickets/bridge/8x-01-szalpha-wrapper-cct.md`** (the fork wrapper + Base mirror + CCT `BurnMintTokenPool`; build-only, no INFLOW) and ran **TWO adversarial-spec critic rounds** (junior/spec-fidelity/ref-verifier/qa/security). **Both rounds: ZERO spec gaps** (spec-fidelity FAITHFUL — redeem-always-callable, throttle-in-§11, treasury/M2 deferred, one-token, APR-is-CRE-computed all honored). **R1** added the pooled-staker model, the Base mirror, the S1–S11 security table, and the expanded test matrix. **R2** converged on one build-critical fix — the `deposit`-vs-ERC20-`mint` path collision (S2 forbade user deposits as written) → split into a user-facing `deposit` (payable, stakes + mints shares via the wrapper's internal path) vs the pool-only CCT `mint`/`burn` (inherit `BurnMintERC20.grantMintAndBurnRoles`); plus tightened S4 to BOTH stake directions w/ distinct errors (`addStake` silent-fail = fund-loss is the more critical), pinned dead-shares to `1e3`, locked the Base mirror to a separate `SzAlphaMirror.sol`, enumerated all 5 deploy-time address asserts, pinned timelock delays (≥48h ops / ≥7d upgrade), and added 6 named test skeletons. Ref-verifier re-confirmed every signature/struct/selector/address against `reference/`. **No 3rd re-fan** (R2 fixes were clarifications of already-intended surface, README §3a). **DEC-02 cleared ⇒ ticket queued for cold-build.** Real-world deploy prereq only: subnet/validator registration (the `NETUID`/`VALIDATOR_HOTKEY` literals are `initialize` args; the build proceeds on fixtures). NOT yet cold-built (this window authored + hardened the ticket; no contract code). **NEXT = cold-build `8x-01` / item 10 / 8-Bx — superintendent to pick.**
- **2026-06-09** (**item 9 — `ZipRedemptionQueue`; BUILT-VERIFIED + KEPT**) — Authored `tickets/sodo/9-zip-redemption-queue.md` (the SENIOR zipUSD→USDC par epoch queue — the inverse of the WOOF-06 mint) through the full harness: verified the "Model from" against `reference/erc7540-reference` + `reference/maple-withdrawal-manager` + `ESynth.sol` (the own-balance burn seam) directly → drafted (build-only; the redeem UI folds into the post-deploy frontend sweep) → **5-critic fanout** (junior/spec-fidelity/ref-verifier/qa/security). **Spec-fidelity FAITHFUL** (par-not-NAV, no-cancel, immutable-non-renounced-controller all honored; confirmed the inbound obligation row 317 [warehouse REDEEM/REPAY] is mine and the sidecar row 272 belongs to the *baal-spec-§8 freeze/rotation module*, NOT this queue — annotated row 272). **Two genuine bugs caught + fixed in the ticket BEFORE build:** (1) **div-by-zero** — a 100%-fill epoch drove the cumulative factor to 0 → next request divided by 0 (junior-dev); fixed with an **`era` counter** (full drain bumps `era` + resets `cumRemaining`; `eraAt<era ⇒ fully filled`). (2) **CRITICAL value-destruction** (security) — the era bump keyed on `filledShares==pending`, but `filledShares` is always a multiple of `scaleUp` while `pending` (zipUSD) can carry sub-`scaleUp` dust → the era could **never** bump → `cumRemaining` decays to 1 → zipUSD burned with no claimable USDC, *all stated invariants still passing*; fixed by **`require(shares % scaleUp == 0)`** in `requestRedeem` (par redemption is whole-USDC-unit anyway) — kills both this and the sub-`scaleUp` dust-trap. Also folded qa hardening (the **ghost-accumulator solvency harness** `ghost_totalPaid <= ghost_totalDelivered`; donation-tolerant `zipBalance >= totalPending`; redeem/withdraw zero guards; cross-era carry-forward + over-funded + boundary tests; `block.timestamp`-anchored epoch clock) and ref-verifier wording fixes (the `burnFrom==sender` branch; line drifts). **Spec edit (faithful clarification, not a mechanism change):** added the whole-USDC-unit redemption granularity note to `claude-zipcode.md §6.1`. **Cold-build (fresh subagent), ZERO load-bearing guesses, independently re-ran green: 44/44** (37 unit + 5 invariant/fuzz over 128k calls + 2 Base-fork: real `WarehouseAdminModule` REPAY→settle, real `ESynth` `totalSupply` drop, KR-2 no-sweep ABI negative), **468/468 total no regression** (`forge test`; fork tests resolved + passed). One build-discovered test-construction nuance folded (preview-parity must force a realize when the fill increment floors to 0). Code `contracts/src/supply/ZipRedemptionQueue.sol` + `contracts/src/interfaces/euler/IZipUSD.sol` + `contracts/test/ZipRedemptionQueue.t.sol`. `reports/9-report.md`. **NEXT = item 10 (deploy/wiring) / 8x bridge / 8-Bx — next kickoff to pick (user-directed 2026-06-09).**
- **SUPERINTENDENT REVIEW 2026-06-09 — ACCEPTED, ON-TRACK.** Independent skeptical pass (all six checks): re-ran 44/44 myself; confirmed the fork tests genuinely fork + drive the real `WarehouseAdminModule` REPAY→settle→real `ESynth.burn` (totalSupply drop, no allowance/capacity — empirically proves the burn seam), the invariant ghost accumulators are real, and the global-factor math is consistent across mixed join points/eras. Ratified the report's 5 design calls (clean-room / global-factor+era / whole-unit guard / distinct CRE-02 controller / REPAY-funded). Seam row 324 DISCHARGED (fork-proven); rows 325/326 OPEN for item-10. **Fixed the one stale corner: `claude-zipcode.md §6.1` step 1/3 reconciled to the as-built REPAY-funded own-balance read (was "freeable USDC from the venue pool").**
- **COMMIT 2026-06-09 — the whole tree is now git-committed at `13f9128`** (branch `snapshot/recycle-rework`; user-directed). This **supersedes every per-row "NOT git-committed" / "whole tree untracked" note above** (item 9, 8-Bw, the 8-B* engine windows, the doc prunes) — they were all snapshotted in `13f9128`.
...
- **2026-06-08** (**Exit Gate + szipUSD BUILT-VERIFIED — the junior share + the windowed exit valve**) — *[EXIT
  ...
  `reports/baal-spec.md §4/§5/§7`, verified vs `reference/Baal` (ragequit:619 burns from `_msgSender()`; mintLoot:814 /
...
- **2026-06-08** (superintendent spec edit — **structural-freeze lock + `DefaultCoordinator` shrink**, user-directed) —
  ...
  Report: `reports/structural-freeze-shrink-report.md`. No §17 reopened (the freeze model was already locked
  2026-06-05/06-07; this removes a stale residual framing, not a decision). `forge build` still clean.
- **2026-06-08** (superintendent review — **8-B4 zero-guess gate CLOSED**) — Reviewed the 8-B4 zero-guess gate report
  (the deferred fresh-subagent gate, now run; report since folded into the ticket + deleted). **Verdict: accept — gate did its job.** Independent from-ticket-alone
...
- **2026-06-07** (**`SzipNavOracle` (8-B4) BUILT-VERIFIED — the szipUSD NAV-per-share oracle**) — Authored
  ...
  `reports/baal-spec.md §3`; fanned out 5 critics (junior-dev/spec-fidelity/ref-verifier/qa/security). **spec-fidelity =
  ...
  **Triage:** the only spec gaps were the 3 fixed above (§4.4 reportType 7 / §7 authorities+denominator / §12
  ...
  formally close the zero-guess gate if desired. **(Update: the gate WAS run + CLOSED 2026-06-08 — see the entry above.)**
  `reports/8-B4-report.md` written. **NEXT = the Exit Gate +
  szipUSD** (`reports/baal-spec.md §4/§5`) — reads `navEntry`/`navExit`/`poke`, wired via `setShareToken`.
- **2026-06-08** (superintendent review of `SzipNavOracle` (8-B4)) — **VERDICT: on-track; Exit Gate + szipUSD
  ...
  **non-blocking** open item: a fresh window MAY re-materialize from `tickets/sodo/8-B4-szip-nav-oracle.md` alone to
  ...
  window. **NEXT = Exit Gate + szipUSD** (`reports/baal-spec.md §4/§5`); inbound obligations it must discharge: the
...
- **2026-06-08** (superintendent review of 8-B1 substrate scaffold) — **VERDICT: on-track; accepted.** Went through
  ...
  the two-tier authority spec edit landed at `claude-zipcode.md §4.5 item-0` (+ `reports/baal-spec.md 8-B1`), **user-ratified**
...
- **2026-06-06** (superintendent — szipUSD-redesign review + authored **8-S2b** two-Safe custody w/ user) —
  ...
  **8-Bw module mechanism RESOLVED 2026-06-06 → B: Zodiac Roles Modifier v2** (user-ratified after a 7-repo +
  web/audit research pass; the 7 Gnosis Guild repos cloned to `reference/`; research doc deleted — decision built
  in 8-Bw). Decisive hard truth: no external guard contains an enabled module
...
- **2026-06-06** (item **8-S3** — `audit/1` re-derivation; spec EDIT, not a build ticket) — Re-derived
  ...
  COMPLETE.** Wrote `reports/8-S3-report.md`. **Follow-up same day (user-directed): EXCISED the stale
...
- **2026-06-06** (item 8 REDESIGN — user-directed; WOOF-07 was the WRONG vault) — The 2026-06-05 WOOF-07 +
  ...
  szipUSD is NOT that. szipUSD is the **auto-compounder junior NAV vault** (`pending-docs/auto-compounder.md` +
...
- **2026-06-05** (borrower-model rework — STEP 1, SPEC EDIT only; superintendent-released, did NOT author a ticket) —
  ...
  Wrote `reports/borrower-model-spec-report.md`. `contracts/` untouched (no cold-build — spec edit). **NEXT: Step 2 —
...
- **2026-06-07** (**8-B1 BUILT-VERIFIED — the szipUSD Baal substrate scaffold**) — Authored + materialized the
  ...
  `baal.avatar()`** (fail-closed). Spec edits: `claude-zipcode.md §4.5 item-0` + `reports/baal-spec.md 8-B1` (rewrote the
