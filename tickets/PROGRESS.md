...
> - **Spec artifacts:** `claude-zipcode.md` = canonical (integrated). **`reports/design/baal-spec.md` = the build-grade companion
...
>   **NEXT (cold-build, superintendent to pick the order): `8x-01` szALPHA bridge (ticket ready NOW) · `8x-02` xALPHA-APR CRE
>   (under CRE-03, after 8x-01) · item 10 (deploy/wiring) · 8-Bx `LienXAlphaEscrow` (M1-adjacent).** Remaining M1 on-chain (contract track): item 10 (deploy/wiring), 8x bridge (`8x-01`, ticket ready),
>   8-Bx `LienXAlphaEscrow` (M1-adjacent); the **loss side** (DefaultCoordinator) is M2. 8-B11 (CRE robot) + 8-B12 (monitoring)
>   are off-chain (`claude-zipcode.md §8` / `monitoring.md`). Build order: `reports/design/baal-spec.md §13`. *(Superintendent may re-prioritize
...
| 8 | `szipUSD` — **the auto-sodomizer junior NAV vault** (**Baal Moloch-v3 + Zodiac**: Loot share, Safe-held multi-asset basket [zipUSD + xALPHA + zipUSD/xALPHA ICHI LP gauge-farmed on Hydrex + room for several strategies], ragequit in-kind exit, **NAV = tracked/displayed from multiple oracles** (not a redemption primitive), CRE-driven via manager-shaman + Zodiac modules, **LOCK (~30d) + FREEZE** gates). | §4.5 / §6.4 / §11 + `pending-docs/auto-sodomizer.md` + `hydrex.md` | multi-ticket REBUILD (~13: 8-Bw + 8-B1…12) | **TWO-TOKEN REDESIGN INTEGRATED 2026-06-07 (see the CURRENT-STATE banner at top — the model nouns in the left cell are OLD/superseded). NEXT = `8-B1` (substrate scaffold via `BaalAndVaultSummoner`) → `SzipNavOracle` → Exit Gate → engine 8-B5…8-B14. Author from `reports/design/baal-spec.md` §13 + `claude-zipcode.md`; cold-build WOOF-06 once 8-B1/oracle/Gate land.** | **PRIOR WOOF-07 + INFLOW-08 DELETED** (wrong vault: ERC-4626 convert-on-stake over EulerEarn loan-book pool shares). Substrate **Baal+Zodiac decided** (user-reversed the interim 7540 lean — don't need single-asset exit/4626 composability; agents 33–29 in plan). First-loss = **WITHHOLD at-risk zipUSD from ragequit** (not markdown/seizure); frozen capital keeps earning. Spec money-model rewrite (§5/§6.4/§11/§12/§17 + audit/1) + ticket decomposition pending — **design still being managed**. See `~/.claude/plans/don-t-be-dropping-deferrals-quirky-ocean.md` + §4.5 guardrail. |
...
*(**Authoritative 8-B build order + per-component specs = `reports/design/baal-spec.md` §13 / §10.8** (2026-06-07 two-token model); 8-B5…8-B13 also in `claude-zipcode.md` §4.5.1. **The rows below were updated to the redesign:** the old 8-B2 mint-shaman + 8-B3 lock/freeze-shaman are **ABSORBED into the Exit Gate**; 8-B4 is now **`SzipNavOracle`** (the **NAV-proportional pricing primitive**, not display-only); **8-B14 buy-and-burn** is added.)*
...
- **2026-06-08** (**Exit Gate + szipUSD BUILT-VERIFIED — the junior share + the windowed exit valve**) — *[EXIT
  ...
  `reports/design/baal-spec.md §4/§5/§7`, verified vs `reference/Baal` (ragequit:619 burns from `_msgSender()`; mintLoot:814 /
...
- **2026-06-07** (**`SzipNavOracle` (8-B4) BUILT-VERIFIED — the szipUSD NAV-per-share oracle**) — Authored
  ...
  `reports/design/baal-spec.md §3`; fanned out 5 critics (junior-dev/spec-fidelity/ref-verifier/qa/security). **spec-fidelity =
  ...
  **Triage:** the only spec gaps were the 3 fixed above (§4.4 reportType 7 / §7 authorities+denominator / §12
  ...
  szipUSD** (`reports/design/baal-spec.md §4/§5`) — reads `navEntry`/`navExit`/`poke`, wired via `setShareToken`.
- **2026-06-08** (superintendent review of `SzipNavOracle` (8-B4)) — **VERDICT: on-track; Exit Gate + szipUSD
  ...
  **non-blocking** open item: a fresh window MAY re-materialize from `tickets/sodo/8-B4-szip-nav-oracle.md` alone to
  ...
  window. **NEXT = Exit Gate + szipUSD** (`reports/design/baal-spec.md §4/§5`); inbound obligations it must discharge: the
...
- **2026-06-08** (superintendent review of 8-B1 substrate scaffold) — **VERDICT: on-track; accepted.** Went through
  ...
  the two-tier authority spec edit landed at `claude-zipcode.md §4.5 item-0` (+ `reports/design/baal-spec.md 8-B1`), **user-ratified**
...
- **2026-06-07** (**8-B1 BUILT-VERIFIED — the szipUSD Baal substrate scaffold**) — Authored + materialized the
  ...
  `baal.avatar()`** (fail-closed). Spec edits: `claude-zipcode.md §4.5 item-0` + `reports/design/baal-spec.md 8-B1` (rewrote the
