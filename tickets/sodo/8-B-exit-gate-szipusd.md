# Exit Gate + szipUSD (§6.4 / §7 / `reports/baal-spec.md §4/§5/§7`) — the junior share + the windowed exit valve
...
bracket; the supply denominator) + **`reports/baal-spec.md §4`** (the one issuance core), **§5** (the patient windowed
...
- **§4.5 item-0 / `reports/baal-spec.md` 8-B1** — the two-tier authority model: the Gate's `manager(2)` is granted by the
...
- **§11 / `reports/baal-spec.md §8/§9`** — the freeze is the **non-ragequittable sidecar** (committed equity); the Gate
...
- **`IBaal`** — the **kept** `contracts/src/interfaces/baal/IBaal.sol` (verified 2026-06-06 vs
  ...
  - **`burnLoot(address[] from, uint256[] amount)`** (`Baal.sol:834`, `baalOrManagerOnly`) — the **paired-burn**
    ...
    window** (`reports/baal-spec.md §18.2`). Used by `burnFor` (§7), NOT by the windowed exit.
...
- **Do NOT expose raw `ragequit` to depositors, and do NOT make szipUSD soulbound/transfer-gated.** The whole
  ...
  hence the sole ragequit caller, hence controls *when* exits happen (§6.4 item 1, `reports/baal-spec.md §2.3`).
