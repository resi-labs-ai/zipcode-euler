# 8-B4 — `SzipNavOracle` (§7 / `reports/baal-spec.md §3`) — the szipUSD NAV-per-share oracle
...
> **NEXT after this:** the Exit Gate + szipUSD (`reports/baal-spec.md §4/§5`).
...
the two-layer xALPHA mark) + **`reports/baal-spec.md §3`** (the build-grade per-leg source table, composition/TWAP/safety
...
- **§4.5 / §4.5.1 / `reports/baal-spec.md §10`** — the basket the oracle marks (zipUSD + xALPHA + the zipUSD/xALPHA ICHI LP
  gauge-farmed on Hydrex + transient USDC/HYDX/oHYDX) and the two Safes (main = free equity / sidecar = committed).
- **§11 / §4.6 / `reports/baal-spec.md §9`** — the **provision-that-recovers**: the `DefaultCoordinator` (M2) writes a
...
- **xALPHA two-layer mark** (`reports/baal-spec.md §3.2` + `tickets/bridge/8x-01-szalpha-wrapper-cct.md`): `xAlphaUSD = exchangeRate ×
...
- **oHYDX intrinsic** — `contracts/src/interfaces/hydrex/IOptionToken.sol` (kept): `discount() returns (uint256)`
  ...
  `discount()` is in the stub; if absent, add it** (cited in `reports/baal-spec.md §10.8` 8-B8).
...
- **Do NOT push any quantity or any on-chain-readable leg price.** `reportType 7` carries **only** `alphaUSD` and
  ...
  oHYDX `discount`, and the ICHI reserves are **read on-chain, never trusted from a push** (`reports/baal-spec.md §3.2`).
...
- **Set-once, `onlyOwner`, frozen by the deploy-time renounce** (deploy-order: szipUSD/Gate, the LP/gauge, the
  ...
  *(ZERO-GUESS GATE 8-B4: an independent re-materialization omitted these setter zero-checks — they were not pinned
  ...
  - **`setEngineSafe(address engineSafe_)`** — the 8-B14 buy-and-burn Safe whose **transient pre-burn szipUSD** is
    excluded from the denominator (`reports/baal-spec.md §2.3`/§3.3). While unset, the pending-burn adjustment is **0**.
...
*Spot NAV composition (the heart — `reports/baal-spec.md §3.3`)*
...
- **Acceptance (integration — owned here, satisfied downstream):** the **Exit Gate** (`reports/baal-spec.md §4/§5`) reads
...
Downstream consumers: the **Exit Gate + szipUSD** (`reports/baal-spec.md §4/§5`), **WOOF-06** (the zap re-author),
...
1. **Exit Gate + szipUSD (next item, `reports/baal-spec.md §4/§5`):** issue NAV-proportionally off `navEntry()` (round down),
   exit/window-RQ at `navExit()`, and `poke()` the accumulator before reading; `szipUSD` is wired via `setShareToken`.
2. **8-B14 buy-and-burn (`reports/baal-spec.md §7`):** the engine Safe holding transient pre-burn szipUSD is wired via
