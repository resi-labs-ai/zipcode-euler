...
- **Cross-protocol tick-math assumption.** The header asserts Algebra uses the identical X96 tick math as UniV3, so this code is correct for the Algebra HYDX/USDC pool. True for Algebra Integral (same Q64.96 sqrt-price encoding), but it is *the* cross-protocol assumption the LP valuation rests on — worth a one-line confirmation against the live pool's `globalState` price vs `getSqrtRatioAtTick(tick)`.
