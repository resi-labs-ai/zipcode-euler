...
4. Fail-closed: unsupported pair / zero supply → `PriceOracle_NotSupported`; no plugin → `NoPlugin` (ctor); short plugin history → `LpTwapHistoryTooShort` (self-healing at `readyAt`) — trustless, no CRE dependency.
