...
face an EVK `EulerRouter` resolves the LP collateral through — THE farm-utility LP oracle (its CRE-push twin
`SzipFarmUtilityLpOracle` was deleted: even a trusted pusher composed the mark from spot `getTotalAmounts`, the
manipulable surface this adapter prices out; a replaced plugin now pauses the farm loop until history refills —
halt-over-degrade, ratified). Exercised by `AlgebraIchiFairLpOracle.t.sol` (base-fork tests vs the live HYDX/USDC
vault `0xfF8B…73f7`).
...
| I-9 | **Both history halts are typed + probed** — depth AND freshness; the freshness half is the 2026-08-04 addition, because Algebra extrapolates an empty window from the newest stored point at the live tick, making the average equal spot while the depth gate passes. **F8 history halt is typed + probed** — a replaced/reset plugin whose ring history under-covers `twapWindow` makes `getQuote` revert the lib's `LpTwapHistoryTooShort(plugin, readyAt)` (EVK borrows/health checks against the LP halt, then self-heal at `readyAt`); `twapStatus()` is the non-reverting probe and agrees with the gate on the live plugin | Yes | **`test_fork_underCoverageWindow_failsClosed`** (typed revert, `readyAt` recomputed from the live ring buffer), **`test_fork_twapStatus_matches_gate`**, lib-level **`test_fairReserves_revert_historyTooShort`** / `_historyGate_wrappedRing` / **`test_historyStatus_shapes`** |
...
| `LpTwapHistoryTooShort` (lib, F8) | `IchiAlgebraFairReserves.fairReserves` history-DEPTH gate; probed via `twapStatus()` | `test_fork_underCoverageWindow_failsClosed`, `test_fairReserves_revert_historyTooShort`, `_historyGate_wrappedRing` |
| `LpTwapNoRecentTimepoint` (lib, 2026-08-04) | the FRESHNESS gate — nothing recorded inside `window` means the mean tick IS spot; probed via `twapStatus()` | `test_fairReserves_revert_noRecentTimepoint`, `test_historyStatus_shapes` (quiet-feed + one-point-inside cases) |
...
4. Fail-closed: unsupported pair / zero supply → `PriceOracle_NotSupported`; no plugin → `NoPlugin` (ctor); short plugin history → `LpTwapHistoryTooShort` (self-healing at `readyAt`); a plugin that has stopped recording → `LpTwapNoRecentTimepoint` (does NOT self-heal — the gap only grows) — trustless, no CRE dependency.
