...
- **Fail-closed reverts** — `NoPlugin`, `PluginNotReady` (the ADV-02 `isInitialized()` read-time gate), `LpTwapHistoryTooShort(plugin, readyAt)` (the audit-F8 history-DEPTH gate: a replaced/reset plugin whose ring buffer covers less than `window` halts with a typed, self-healing error naming the plugin and the resume time), `LpTwapNoRecentTimepoint(plugin, newest)` (the FRESHNESS gate, added 2026-08-04 — see below), and `BadTimepoints` reject a missing/uninitialized/under-covered/stale/malformed TWAP source rather than returning a manipulable spot value. Both history gates read the ring's bounds via `_ringBounds` (`timepointIndex()` is always the newest; the oldest is slot 0 pre-wrap / slot head+1 wrapped). `historyStatus(vault, window)` is the non-reverting probe of BOTH gates (surfaced as `SzipNavOracle.lpTwapStatus()` / `AlgebraIchiFairLpOracle.twapStatus()` for CRE + front-ends).
- **The two history gates guard opposite ends of the ring, and only one self-heals.** F8's depth gate asks whether the window is COVERED — enough history exists — and a fresh plugin heals it unaided within `window`. The freshness gate asks whether the window is POPULATED: Algebra's `getTimepoints` extrapolates BOTH requested endpoints from the newest stored timepoint using the pool's live tick, so if nothing was recorded inside the window then `cum(now) - cum(now-window) == tick * window` and `_meanTick` returns SPOT with zero averaging — the manipulation resistance this library exists for is silently gone, and the depth gate passes because the oldest slot is ancient. That state does not heal; the gap only grows. It is reachable without touching the plugin pointer: `pluginConfig`'s before-swap bit is a live Hydrex-controlled bitmask, and clearing it leaves the plugin attached and `isInitialized()` true while no swap records a price. Measured on the Base fork at block 47096000, the live HYDX/USDC pool's newest timepoint was already 1520s old against the 3600s window — 42% of the average extrapolated, with the probe reporting ready. The gate requires at least one timepoint strictly inside `window`, deriving its bound from `window` rather than adding a tunable: a pool liquid enough to collateralise against trades at least once per window, and a quieter one should halt rather than price off spot. Cost is one extra `timepoints()` staticcall against already-warm storage.
...
| Mock-plugin fail-closed unit | 8 | `NoPlugin`, `PluginNotReady` (ctor + read), `BadTimepoints`, `LpTwapHistoryTooShort` (pre-wrap + wrapped ring), `LpTwapNoRecentTimepoint` (ample depth, quiet feed), `historyStatus` shapes incl. the quiet-feed and one-point-inside cases |
...
- ~~No TWAP cardinality/staleness assertion~~ — **CLOSED** (audit F8 + the 2026-08-04 freshness gate): the depth gate reverts `LpTwapHistoryTooShort(plugin, readyAt)` when the populated span is shorter than `window`, and the freshness gate reverts `LpTwapNoRecentTimepoint(plugin, newest)` when nothing was recorded inside it; `historyStatus` is the non-reverting probe of both. What remains genuinely un-assertable on-chain is the *economics* of the pool's depth vs. the window, not the span.
