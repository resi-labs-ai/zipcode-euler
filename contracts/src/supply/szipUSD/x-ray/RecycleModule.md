...
| I-3 | **cumulative divert bound** — total diverted ≤ the markdown, enforced by the hole ITSELF: each `divert` retires `provision()` by exactly what it paid, so the live read is the remaining budget. Strict `>` (exact fill allowed, never over-fill). *Rewritten 2026-08-02: the per-epoch tally this used to describe is gone, along with its reset-on-remark and stale-value-key clauses — both were properties of a counter that only existed because `divert` left the hole standing.* | Yes | **`test_SEC09_cumulative_overfill_blocked`**, `_exact_cumulative_fill_and_one_wei_over`, `_divert_settles_provision_by_the_amount_paid` |
| I-3b | **atomic settlement** — `divert` retires the markdown in the same tx that pays it, for EXACTLY the amount paid, and fails closed if the coordinator is unwired or the lien cannot absorb it. A `WrittenOff` lien is a legal settle target, which is what keeps the understatement from being permanent | Yes | **`test_PROVEN_F1_divert_settles_the_markdown_atomically`** (`ClaimVerification.t.sol`), `test_divert_reverts_when_coordinator_unwired`, `test_divert_reverts_when_nothing_settles` |
...
| I-5 | **CEI / effects-before-interaction** (reentrancy safety without a guard) — the ledger decrement lands before the value-moving execs | Yes | **`test_integrated_decrement_before_exec`**, `test_divert_decrement_before_exec` |
