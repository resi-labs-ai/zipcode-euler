...
`test/SzipBuyBurnModule.t.sol`: **57 functions, all passing** (~46 module + 8 CTR-01 receiver + 1 fork; the CTR-01
...
| I-8 | **atomic post / idempotent cancel — now closed for the SILENT-false case too (F9)** — approve + presign are one tx via the checked `_exec` (the fleet idiom this module previously missed): a failed presign — whether it REVERTS or the Safe swallows it and returns `false` — rolls back the approve and reverts (`ExecFailed` or the bubbled inner reason, e.g. CoW's "GPv2: cannot presign order"). A failed CANCEL likewise reverts and KEEPS the bid state, instead of wiping it while the presign stays live on the settlement (the believed-cancelled shape). Cancel no-ops when no live bid | Yes | **`test_atomicity_second_exec_revert_rolls_back`**, **`test_F9_postBid_silentExecFalse_reverts_no_phantom_bid`**, **`test_F9_cancelBid_silentExecFalse_reverts_keeps_state`**, `test_exec_discipline_postBid_calls`/`_cancelBid_calls` |
...
| X-1 | §10.1 residual: operator sizes the 3 order fields — bounded, not theft | **No** | recipient pinned to the Safe + the price/cap/coverage/freshness gates cap it; a compromised Timelock could redirect avatar/target via the inherited `setAvatar`/`setTarget` (accepted, same Timelock governs all) — but F9 narrowed the blast radius: `setJuniorTrancheEngine` now syncs avatar/target in lockstep (the syncing-sibling idiom), and a divergence created via the inherited setters makes every post/cancel revert LOUD at CoW's `owner == msg.sender` presign check (bubbled by the checked `_exec`) instead of silently phantom-bidding. Build-phase wiring re-points are Timelock-only and closed by the pre-prod immutable re-freeze; the four bid-dereferenced setters (settlement/relayer/usdc/engine) refuse a re-point under a live bid (`BidAlreadyLive`) so a rewire can't strand a presign/allowance/receiver. The `setJuniorTrancheEngine`↔oracle-free-Safe binding (the undercovered-fill window's free-side-USDC premise) remains an off-chain wiring convention — accepted X-1, governance-bounded, with the deployers now asserting `avatar == target == engine` at wire-time (`SeamEngineAvatar`) |
...
| `setSettlement`/`setVaultRelayer`/`setUsdc`/`setJuniorTrancheEngine` no-live-bid re-check (`BidAlreadyLive`) — the setters a live bid dereferences or names refuse a re-point, so a rewire can't strand the old presign/allowance/receiver | `test_SUPPLYADV05_wiring_setters_reject_rewire_under_live_bid` |
| `setSettlement` cache refresh (F9) — re-reads `vaultRelayer` + `domainSeparator` off the new settlement (both `immutable` there; the domain hashes the settlement's own address, so a re-point always invalidates the cached pair — stale domain = unfillable uids) | `test_wiring_setters_onlyOwner_effect_and_zeroGuard` (asserted against a distinct-value `MockSettlement`) |
| `ExecFailed` / bubbled inner revert (F9) — every Safe exec checked via `_exec`; silent-false posts record nothing, silent-false cancels keep state | `test_F9_postBid_silentExecFalse_reverts_no_phantom_bid`, `test_F9_cancelBid_silentExecFalse_reverts_keeps_state` |
...
- **The 6 wiring setters — now covered** — `test_wiring_setters_onlyOwner_effect_and_zeroGuard` exercises
  ...
  the governed params, **every mutator on the value-out module is now exercised**. F9 brought the setter into line
  with the fleet: `setJuniorTrancheEngine` now syncs avatar/target in lockstep (asserted) and refuses a re-point
  under a live bid; `setSettlement` refreshes the cached relayer + domain off the new settlement (asserted).
...
|---|---|---|
| Module unit | ~42 | setUp/guards/SEC-14/15, all 7 wiring setters (incl. the no-live-bid re-check on settlement/relayer/usdc/engine + the F9 ExecFailed + cache-refresh regressions), single-bid, price-bound boundaries, caps/killswitch, coverage gate, the SEC-13 freshness-fence cluster (4), freshness gate, exec-discipline + atomicity, the uid KAT + typehash |
...
All **57 pass** (`forge test --match-path test/SzipBuyBurnModule.t.sol`). The exit-safety validations, the uid
