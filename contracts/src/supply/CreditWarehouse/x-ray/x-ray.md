...
2. **`warehouseSafe` ↔ `roles.avatar()` parity break** — these are independent slots; a one-sided re-point would brick SUPPLY/REDEEM (fail-closed) and leave REPAY live against the wrong Safe (no "from" to scope). **Now caught on-chain at all three pairing sites**: the constructor, `setRoles`, and `setWarehouseSafe` each revert `AvatarMismatch` on a mismatch (re-points are order-dependent: pair the modifier to the Safe first).
...
- **`warehouseSafe` ↔ `avatar` parity is enforced on-chain** &nbsp;[[X-2/I-4](invariants.md#x-2-now-enforced-on-chain--see-i-4)] — the constructor, `setRoles`, and `setWarehouseSafe` each read `roles.avatar()` and revert `AvatarMismatch` on a mismatch, so a one-sided re-point of either slot through the adapter can't be saved (the `setRoles`/ctor legs close the REPAY wrong-Safe drain — REPAY has no "from" to scope). Order-dependent re-points documented in `docs/roles.md`; proven by the `test_Parity_*` set + `test_Ctor_RevertsOnAvatarMismatch`.
...
- **`warehouseSafe ↔ roles.avatar()` parity — NOW ENFORCED ON-CHAIN AT ALL THREE PAIRING SITES** — the contract's own documented #1 hazard is closed in code: the constructor, `setRoles`, and `setWarehouseSafe` each revert `AvatarMismatch` unless `roles.avatar() == warehouseSafe`. The ctor/`setRoles` legs close the pipeline-analysis warning (a wrong-avatar Roles kept REPAY live against the wrong Safe — no "from" to scope). Proven by `test_Parity_OneSidedRepoint_RevertsAtSetter` / `test_Parity_PairedRepoint_SetAvatarFirst_Succeeds` (warehouseSafe leg), `test_Ctor_RevertsOnAvatarMismatch` / `test_Parity_SetRoles_WrongAvatar_Reverts` / `test_Parity_SetRoles_MatchingAvatar_Succeeds` (ctor/roles legs). The scope-level rejection backstop stays covered by `test_Scope_PinsParams_DepositReceiver`. Runbook + invariant documented in `docs/roles.md`.
...
4. Avatar parity is enforced on-chain at the ctor, `setRoles`, AND `setWarehouseSafe` (`AvatarMismatch`): re-points are order-dependent (pair the modifier to the Safe first), closing both the X-2 silent-brick hazard and the REPAY wrong-Safe drain; documented in `docs/roles.md`.
