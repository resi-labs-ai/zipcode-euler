...
- **Residual — RESOLVED (2026-06-22):** the evidence was a point-in-time snapshot with no re-validation. `test/interfaces/InterfaceSelectorDrift.t.sol` (Base-fork) now re-checks every consumed selector against live deployed bytecode (proxy-resolving + leading-zero-safe) — run it (`forge test --match-path 'test/interfaces/*' --fork-url base`) to detect drift on demand / in CI. First run: **no live selector drift** (15/15); it surfaced two stale *citations* (not drift) — `IBaal`'s source-address mislabel (the summoner impl, not the Baal template — **corrected**) and the Hydrex `IVoter` impl address (upgraded since snapshot; all 6 consumed selectors survive — comment stale only). The 964 Subtensor precompiles remain snapshot-trusted (no public fork).
