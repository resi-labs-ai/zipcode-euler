...
wiring; holds no funds, transfers nothing. Exercised by `SeniorNavAggregator.t.sol` — **25 unit tests** (mock
...
| I-13 | **per-silo failure isolation** — one broken pool (reverting views) cannot brick the Σ: its pair counts ZERO (understated backing = a breaker that trips early, the safe direction) and the skip is reported, never silent — by the counter for THAT leg. Each aggregate has its own (`unreadablePairs` = senior, `unreadableIlliquidPairs` = illiquid, `unreadableActivePairs` = active-only), because the legs break independently: EulerEarn views do NOT answer from own storage — both legs fan out over `withdrawQueue`, and `maxWithdraw` (illiquid only) additionally simulates withdrawal per strategy, so a pool can break the illiquid Σ while the senior Σ stays complete (pre-fix, `unreadablePairs()` alone read that state as fully healthy). A future third-party pool is upgradeable by its own governance and silos can never leave `allSiloIds()` — without isolation one post-retirement upgrade blinds the solvency views forever. The strict per-silo getters keep the loud revert for diagnosis. The keeper probe (`solvency_probe_job.go`) reads all three counters | Yes | **`test_broken_pool_skipped_and_reported_not_bricking`** (skip + report + conservative ratio + self-heal), **`test_maxWithdraw_only_outage_visible_on_illiquid_counter`** (the per-leg regression), **`test_broken_pool_strict_getter_still_reverts`** |
