...
| I-2 | **admission guards** — `ZeroSiloId`, `DuplicateSilo`, `ZeroAddress` on each of 9 fields | Yes | **`test_addSilo_zeroId_reverts`**, `_duplicate_reverts`, `_zeroAddress_eachField_reverts` (loops all 9) |
...
| I-7 | **retire stops routing, keeps record, clears sentinel if current** | Yes | **`test_retireSilo_stopsRouting_keepsRecord`**, `_retireSilo_nonCurrent_keepsCurrent` |
