...
/// @notice A minimal 18-dp ERC20 stand-in for a lien key. The registry's `_writePrice` strict-reads `decimals()`
...
/// @notice Proves the CTR-16 posture against the REAL `ZipcodeOracleRegistry` (a `ReceiverTemplate` push-cache), in
///         four classes:
///           1. IDENTITY SEALED — after the seal, `getExpectedAuthor()`/`getExpectedWorkflowName()` are non-zero.
///           2. BEHAVIORAL fail-closed — an unsealed receiver ACCEPTS a wrong-identity revaluation push (the dormant
...
///         No fork: the registry ctor reads `quote.decimals()` (6-dp `MockUSDC`) and `_writePrice` strict-reads the
///         lien key's `decimals()` (18-dp `MockLp18`).
contract CTR16ReceiverIdentityTest is Test {
    LpGateHarness internal harness;
    MockUSDC internal usdc;
    ZipcodeOracleRegistry internal receiver;

    address internal FORWARDER = makeAddr("forwarder");
    address internal LP_TOKEN = address(new MockLp18()); // live 18-dp key — `_writePrice` strict-reads its decimals
    address internal WORKFLOW_OWNER = makeAddr("workflowOwner");

    uint256 internal constant VALIDITY = 365 days;
    uint8 internal constant REVALUATION = 3;
    ...
    function setUp() public {
        ...
        receiver = new ZipcodeOracleRegistry(FORWARDER, address(usdc), VALIDITY);
        assertEq(receiver.owner(), address(this), "test owns receiver");
    }

    /// @dev Replicates the deploy's `_sealIdentity(receiver, name)`: set author + workflowName.
    function _seal(ZipcodeOracleRegistry o, string memory name) internal {
    ...
    /// @dev A well-formed single-key `REVALUATION` report envelope (`abi.encode(uint8 reportType, abi.encode(...))`).
    function _revalReport(uint256 mark, uint32 ts) internal view returns (bytes memory) {
        address[] memory liens = new address[](1);
        liens[0] = LP_TOKEN;
        uint256[] memory prices = new uint256[](1);
        prices[0] = mark;
        return abi.encode(uint8(REVALUATION), abi.encode(liens, prices, ts));
    }
    ...
    function test_IdentitySealed_AfterSeal() public {
        assertEq(receiver.getExpectedAuthor(), address(0), "pre-seal: author dormant");
        assertEq(receiver.getExpectedWorkflowName(), bytes10(0), "pre-seal: name dormant");
        _seal(receiver, NAME_A);
        assertEq(receiver.getExpectedAuthor(), WORKFLOW_OWNER, "author sealed");
        assertTrue(receiver.getExpectedWorkflowName() != bytes10(0), "name sealed");
    }
    ...
    /// @dev The vuln (dormant): NO identity ⇒ a wrong-identity revaluation from the (shared) Forwarder is ACCEPTED
    ///      and writes the cache. Any co-tenant workflow clearing the Forwarder can push the mark.
    function test_Behavioral_DormantAcceptsWrongIdentity() public {
        assertEq(receiver.getExpectedAuthor(), address(0), "identity dormant");

        vm.warp(VALIDITY + 100); // so a `ts` in range is non-zero and <= now
        uint32 ts = uint32(block.timestamp);
        vm.prank(FORWARDER);
        receiver.onReport(_metadata(bytes10("whatever"), makeAddr("strangerDaemon")), _revalReport(15e6, ts));

        (uint208 price, uint48 cachedTs) = receiver.cache(LP_TOKEN);
    ...
    function test_Behavioral_SealedRejectsWrongName() public {
        _seal(receiver, NAME_A);
        bytes10 sealedName = receiver.getExpectedWorkflowName();
        ...
        receiver.onReport(_metadata(wrongName, WORKFLOW_OWNER), _revalReport(15e6, ts));

        (, uint48 cachedTs) = receiver.cache(LP_TOKEN);
    ...
    function test_Behavioral_SealedAcceptsCorrectIdentity() public {
        ...
        uint32 ts = uint32(block.timestamp);
        vm.prank(FORWARDER);
        receiver.onReport(_metadata(sealedName, WORKFLOW_OWNER), _revalReport(15e6, ts));
    ...
    function test_PreGate_RevertsWhenAuthorUnset() public {
        ...
        receiver.setExpectedWorkflowName(NAME_A);
        assertEq(receiver.getExpectedAuthor(), address(0), "author unset");
        vm.expectRevert(
            abi.encodeWithSelector(ZipcodeDeployAsserts.ReceiverIdentityNotWired.selector, address(receiver))
        );
        harness.gate(address(receiver));
    }

    function test_PreGate_RevertsWhenNameUnset() public {
        receiver.setExpectedAuthor(WORKFLOW_OWNER); // author only
        assertEq(receiver.getExpectedWorkflowName(), bytes10(0), "name unset");
        vm.expectRevert(
            abi.encodeWithSelector(ZipcodeDeployAsserts.ReceiverIdentityNotWired.selector, address(receiver))
        );
        harness.gate(address(receiver));
    ...
    function test_PreGate_PassesWhenSealed() public {
        _seal(receiver, NAME_A);
        harness.gate(address(receiver)); // no revert
    }
    ...
    function test_K5_PrivilegeSeparation_NameSeparatesSameAuthorDaemons() public {
        ZipcodeOracleRegistry receiverA = receiver; // sealed to daemon-A below
        ZipcodeOracleRegistry receiverB = new ZipcodeOracleRegistry(FORWARDER, address(usdc), VALIDITY);

        _seal(receiverA, NAME_A);
        _seal(receiverB, NAME_B);

        // identical author, distinct on-chain names.
        assertEq(receiverA.getExpectedAuthor(), receiverB.getExpectedAuthor(), "same author (shared deploy wallet)");
        bytes10 nameA = receiverA.getExpectedWorkflowName();
        bytes10 nameB = receiverB.getExpectedWorkflowName();
        ...
        vm.prank(FORWARDER);
        receiverA.onReport(metaA, _revalReport(15e6, ts));
        (uint208 priceA, uint48 tsA) = receiverA.cache(LP_TOKEN);
        ...
        receiverB.onReport(metaA, _revalReport(15e6, ts));
        (, uint48 tsB) = receiverB.cache(LP_TOKEN);
