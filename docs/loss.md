...
* X-2 (build-phase, on-chain=No) — destination integrity is absolute only after the pre-prod immutable re-freeze: the sinks (adminSafe/juniorTrancheSafe) and the wiring are Timelock-settable during the build phase, so the theft-immunity absolute is conditional until that process step ships. The owner has no sweep or pause power. The owner DOES hold one NAV-inflation lever until the re-freeze: re-pointing the coordinator's recycleModule slot lets settleFromJunior erase a loss markdown with no cash moved, because the payment proof lives in RecycleModule, not the coordinator. The re-freeze list must include that slot.
