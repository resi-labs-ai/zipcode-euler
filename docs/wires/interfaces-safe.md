...
- **Consumed by:**
  - `contracts/src/supply/szipUSD/DurationFreezeModule.sol` — drives BOTH Safes via `ISafe(juniorTrancheSafe).execTransactionFromModule(...)` (line 283) and `ISafe(juniorTrancheSidecar).execTransactionFromModule(...)` (line 305) for share rotation. Its inherited single-avatar Zodiac exec is inert; rotation goes through explicit `ISafe(src)` calls (lines 89/110/225).
  - `contracts/script/SummonSubstrate.s.sol` — asserts `ISafe(juniorTrancheSidecar).isOwner(team)` / `ISafe(juniorTrancheSafe).isOwner(team)`; builds the `addOwnerWithThreshold(team,1)` payload that Baal (an enabled module) self-calls via `execTransactionFromModule`; drives the main Safe as owner via `execTransaction`.
