...
- **← `SeniorNavAggregator`** (CTR-05) reads `allSiloIds()` → `getSilo(id).{eePool,warehouseSafe}` to sum senior
  NAV across silos (the struct getter is the canonical read; no per-field getters owed).
