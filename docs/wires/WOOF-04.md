...
- **adapter ↔ EE_POOL (allocator + curator).** The adapter holds **both** EE roles. As **curator** it `submitCap`/`acceptCap`s the new line vault inside `openLine` (bounded to its own minted vault). As **allocator** it `setSupplyQueue` (openLine append **and closeLine prune**, SEC-06), `updateWithdrawQueue` (closeLine withdraw-slot reclaim, CTR-04 — `onlyAllocatorRole` admits the curator), and `reallocate` (fund). Granting these roles is item-10 (§9), not done in this contract.
