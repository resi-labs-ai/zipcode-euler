...
| **P7 loss** | `LienXAlphaEscrow` (coordinator placeholder) → `DefaultCoordinator` → close the cycle via `escrow.setCoordinator` / `coord.setEscrow` (no standing allowance — `_lock` approves the exact amount JIT, LOSS-ADV-01) → `navOracle.setDefaultCoordinator`. Asserts `SeamEscrowCoordinator`. |
