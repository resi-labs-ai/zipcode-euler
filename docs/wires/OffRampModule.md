...
## Role
An engine **Zodiac Module** enabled on the **rq/main Safe** (`avatar == target == juniorTrancheSafe == Baal.avatar()`). It is
...
|---|---|
| `OffRampModule` (`is Module`) | The driver. `setUp(bytes)`-under-`initializer` decodes 5 addresses; `onlyOperator` `requestRedeem(zipAmount)` / `claim(assets)`; private bubbling `_exec`; 4 Timelock (`onlyOwner`) wiring setters. Set-once storage `juniorTrancheSafe`/`operator`/`zipUSD`/`queue`, **not `immutable`** (§18.6 clone fact — a `ModuleProxyFactory` clone shares mastercopy bytecode). |
...
- **It is a `Module`.** `setUp(bytes initParams) public override initializer` decodes
  `(owner, juniorTrancheSafe, operator, zipUSD, queue)` (5 addresses). ORDER is load-bearing: validate **all five nonzero**
  (`ZeroAddress`) and `owner != operator` (`OwnerIsOperator`) FIRST, then set `avatar = target = juniorTrancheSafe`, store the
...
- **`requestRedeem(uint256 zipAmount) external onlyOperator`** — the off-ramp entrypoint. Guards: `zipAmount != 0`
  ...
  **LIVE** off the queue each call, never the hard-coded `1e12` (it is mutable, re-derived on `setTokens`). Then it
  ...
  2. `_exec(queue, IZipRedemptionQueue.requestRedeem(zipAmount, juniorTrancheSafe, juniorTrancheSafe))` — positional
     `(shares, requester, owner)`, so **`requester == owner == juniorTrancheSafe`**;
  3. `_exec(zipUSD, IERC20.approve(queue, 0))` — allowance reset.
  Emits `Redeemed(zipAmount, juniorTrancheSafe)`.
- **`claim(uint256 assets) external onlyOperator`** — the realize leg. Guard `assets != 0` (`ZeroAmount`); one
  `_exec(queue, IZipRedemptionQueue.withdraw(assets, juniorTrancheSafe, juniorTrancheSafe))` (positional `(assets, receiver, requester)`)
  → USDC lands back in the rq Safe (the basket). Emits `Claimed(assets, juniorTrancheSafe)`.
- **How it reaches the queue (via the Safe).** `_exec(to, data)` is private and calls the inherited
  ...
  `msg.sender == juniorTrancheSafe`: this satisfies (a) C4's `onlyRedeemController` (wired to the rq Safe) and (b) the queue's
  `owner == msg.sender` check (since `owner == juniorTrancheSafe`), and (c) the USDC claim/escrow accrues to the rq Safe.
...
- **Timelock-settable wiring (build phase, §17).** Each slot has an `onlyOwner` setter, `ZeroAddress`-guarded,
  emitting `WiringSet(bytes32 slot, address value)`: `setJuniorTrancheSafe` (**also re-points avatar+target in lock-step**),
...
- **Wire + assert `redeemController == juniorTrancheSafe`.** Call `ZipRedemptionQueue.setRedeemController(juniorTrancheSafe)` (Timelock,
  `onlyOwner`) and **assert** the queue's `redeemController` == the rq Safe (the avatar the module drives) — NOT the
  module address. The fork proof is `RedeemRequest.sender == juniorTrancheSafe` after an operator `requestRedeem` through the
...
- **`setJuniorTrancheSafe` moves three slots.** Re-pointing `juniorTrancheSafe` also re-points `avatar` and `target` in lock-step; the
