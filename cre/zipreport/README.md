...
|---|---|---|---|
| `1` Origination | `ZipcodeController.RT_ORIGINATION` (`:47`) | `(bytes32 lienId, bytes32 proofRef, uint256 equityMark, uint16 borrowLTV, uint16 liqLTV, uint256 drawAmount, uint256 cap, bytes32 siloId, uint48 sourceTs)` (`:222`) | `Origination(...)` |
| `2` Draw | `ZipcodeController.RT_DRAW` (`:48`) | `(bytes32 lienId, bytes32 proofRef, uint256 equityMark, uint256 drawAmount, uint48 sourceTs)` (`:266`) | `Draw(...)` |
...
`CoordinatorReportType==8` and `RateReportType==8`; the warehouse
...
`LegAlphaUsd=0`/`LegHydxUsd=1`, `CoordinatorReportType` + `ActionLock..ActionWriteOff`,
`RateReportType`, `WhSupply/WhApprove/WhRedeem/WhRepay`.

> Naming note: where a reportType constant and a builder would collide (`NavLeg`), the builder
> takes the `…Report` suffix (`NavLegReport`) — a Go identifier cannot be both a const and
...
// Push a rate: abi.encode(8, abi.encode(uint256 rate, uint48 ts)).
env, err := zipreport.Rate(new(big.Int).SetUint64(rate), big.NewInt(now))
if err != nil { /* ... */ }
