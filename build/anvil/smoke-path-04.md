# SP-04 — Reservoir borrow / repay (real EVK)

**Intent.** Stand up a real line of credit and utilization on the reservoir EVK market: post LP collateral, borrow
USDC against it, repay, withdraw — all driven by the engine operator through the main Safe.

**Proves.** `ReservoirLoopModule` borrow/repay lifecycle on a real EVK borrow vault; the borrow cap; over-repay
revert; the `ReservoirBorrowGuard` pinning the engine Safe as the sole legal borrower; live debt/collateral views.

**Tier.** Pure on-chain (operator EOA) — but needs token seeding (LP shares + USDC liquidity in the borrow vault).

**Binds to.** `ReservoirLoopModule` `0x61cdc9c8…`, borrow vault `0x1aFc8c64…`, escrow vault `0x8A5FA367…`,
EulerRouter `0x5a451fEB…`, POL ICHI vault `0x07e72E46…` (LP), USDC, EVC `0x5301c7dD…`, `creOperator`.
Source: `contracts/src/supply/szipUSD/ReservoirLoopModule.sol` (`postCollateral`, `borrow` w/ cap, `repay`,
`withdrawCollateral`, `outstandingDebt`, `postedCollateral`), `script/ReservoirMarketDeployer.sol`,
`contracts/src/supply/szipUSD/ReservoirBorrowGuard.sol`, wires `8-B5-ReservoirLoop.md`.
...
**Calls.**
1. `ReservoirLoopModule.postCollateral(<lpAmount>) as creOperator`.
2. `ReservoirLoopModule.borrow(100_000e6) as creOperator`.
3. (negative) `borrow(<over cap>) as creOperator` → revert `CapExceeded`.
4. `ReservoirLoopModule.repay(100_000e6) as creOperator`.
5. (negative) `repay(1) as creOperator` after full repay → EVK `E_RepayTooMuch`.
6. `ReservoirLoopModule.withdrawCollateral(<lpAmount>) as creOperator` (requires debt == 0).
...
- (guard) attacker EOA `0x15d34AAf…` enabled BV as its own controller and called `borrow(1000e6, attacker)` directly → **`NotEngineSafe` (0x455156ee)** from `ReservoirBorrowGuard` on `OP_BORROW`. ✓
