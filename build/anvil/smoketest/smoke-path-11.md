...
(S6); the exact-amount JIT escrow allowance. Sources: `docs/loss.md` E-1/X-1/X-2,
...
**Result.** **PASS** (live fork; `recoveryFloor` 0.5e18; lien `0x8322bc08…`).
...
- **RESOLVE(capitalSlash=600e18)** → `provision` healed to **0**; **capitalSink** xALPHA 0 → **600e18** (realized hole);
  **engine Safe** xALPHA 0 → **400e18** (cohort premium remainder — a prior run routed this to the sidecar; the
