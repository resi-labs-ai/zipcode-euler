...
retracts; the `StaleNav` fence; refuse-wiring-re-point-under-a-live-bid (negative). Sources:
...
→ `BidAlreadyLive`. (+ a wiring setter while a bid is live → reverts, no stranded presign/allowance.)
...
**Result.** **PASS** (live fork; `_harness.sh` seed; `dBps`=100 → `quoteMaxPrice`=990,000 at navExit 1e18).
...
- **(neg) StaleNav:** the pre-seed `postBid` (NAV stale) left no bid — the post-seed bid was the first/only one to
  ...
  replace it (single-bid invariant; selectors: `StaleNav` 0x0c5d582f, `BidAlreadyLive` 0x41666638). ✓
