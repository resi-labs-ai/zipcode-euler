...
and a bare `!= 0` is too loose (EVK `deposit(0,..)` does not revert). **Then the SEC-08 timelock precheck
(audit #10 / kill-list M6):** `if (eulerEarn.timelock() != 0) revert EulerEarnTimelockNonZero();` — read LIVE
