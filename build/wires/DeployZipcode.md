...
| **P9 seal** | `setExpectedAuthor`/`setExpectedWorkflowId` on every `ReceiverTemplate` (controller, registry, warehouse adapter, coord, navOracle, rateOracle) → `ZipcodeDeployAsserts.requireIdentityWired` pre-gate → `transferOwnership(timelock)` everywhere. |
