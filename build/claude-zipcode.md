...
| CRE inbound | `ReceiverTemplate :: onReport(metadata, report) (:78) → _processReport (:119)`; `_decodeMetadata → (workflowId, workflowName, workflowOwner)`; gated on the CRE Forwarder (`s_forwarderAddress`, set at construction `:48`). **As-built correction (supersedes the earlier "immutable / we drop the setter" framing):** `ReceiverTemplate is Ownable`, and the setter is **RETAINED** — `setForwarderAddress`/`setExpectedAuthor`/`setExpectedWorkflowName`/`setExpectedWorkflowId` are all `onlyOwner`, so the Forwarder + the workflow identity are **Timelock-mutable** in the build phase per §17 (re-pointable in an emergency; immutability is the deferred pre-prod lock-down). A zero Forwarder still makes `onReport` permissionless `:82-85`. (So `SzAlphaRateOracle` and every other `ReceiverTemplate` HAS an owner; only the economic knobs are immutable.) **As-built deploy posture (CTR-16):** the seal uses `author`+**per-receiver `workflowName`** and DROPS the shared `workflowId` pin (left `bytes32(0)`) — author+name survive workflow redeploys (no fleet re-seal footgun) and per-receiver names separate the SEPARATE daemons that share the deploy wallet; see §9. | `reference/x402-cre-price-alerts/contracts/interfaces/ReceiverTemplate.sol` | `reference/x402-cre-price-alerts/contracts/interfaces/ReceiverTemplate.sol` |
...
`setExpectedWorkflowName(<this receiver's daemon name>)` on every `ReceiverTemplate` subclass** (`ZipcodeController`,
`ZipcodeOracleRegistry`, `DefaultCoordinator`, `SzipNavOracle`, `SzipFarmUtilityLpOracle`, `SzAlphaRateOracle`,
the per-silo `WarehouseAdminModule`) **and assert `getExpectedAuthor() != 0 && getExpectedWorkflowName() != 0`
on EACH immediately before `transferOwnership(timelock)`, aborting the deploy otherwise** (CTR-16 — the shared
`setExpectedWorkflowId(WORKFLOW_ID)` pin is **DROPPED**; `author`+`workflowName` survive workflow redeploys and the
**per-receiver names** separate the SEPARATE daemons sharing the one deploy wallet, which a shared author cannot). The
workflow-identity check in `ReceiverTemplate.onReport (:88-117)` is enforced **only when these expected values are non-zero**
