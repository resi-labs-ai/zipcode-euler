...
| M3 ✅ | Coverage gate is **post-time only** — a resting CoW buy-burn bid keeps filling after coverage drops below floor (no fill-time re-check). **DOC-RESOLVED 2026-06-16 (SEC-DOC):** intentional, not a breach — buy-burn USDC is free-side engine-Safe value `coverageValue()` already EXCLUDES, so a post-drift fill can't breach the floor; `APP_DATA == 0` forbids a CoW fill-time hook (the hook fix is REJECTED). Documented in `SzipBuyBurnModule` NatDoc + §6.4/§7. | subsystem #5, interconnection-C |
...
| M7 ✅ | `RecycleModule.divert` has **no cumulative tally** vs `provision()` → the same default markdown can be over-filled across calls (per-call ceiling, not cumulative). **RESOLVED 2026-06-15 (SEC-09):** `lastSeenProvision` + `divertedSinceProvisionChange` tally; per-call `:290` check replaced by cumulative `divertedSinceProvisionChange + scaled > hole` (reset on any observed provision re-mark, bumped in effects phase). Last-seen + single-counter (NOT value-keyed) → re-mark-churn-safe; divert still never writes provision. Per-epoch guarantee; cross-re-mark over-supply benign (peg-strengthening, hard-capped by `freeValueAccrued` + trusted CRE). | interconnection-C2 |
| M8 ✅ | `_resolve` heals the full junior provision with **no realized-receipt bound** (unlike `_recovery`) → junior NAV marks up on a "clean resolution" carrying no recovery. **DOC-RESOLVED 2026-06-16 (SEC-DOC):** the full heal IS the ratified §8.4 clean-resolution design (`totalProvision` invariant holds; sole writer). The proposed `capitalSlashAmount <= recoveryProceeds` assert is REJECTED — unit-incoherent (xALPHA bond units vs USD) and `_resolve` has no `recoveryProceeds` param. Header docstring (`:26-28`) reconciled. | interconnection-C3 |
| M9 | Gate manager grant can be **permanently bricked on a re-pointed Baal** if `managerLock==true`. | ref-B (B5) |

### LOW (18) — abbreviated
`requiredCommittedValue` gross-cap can make `covered()` permanently false (bricks release/postBid) · `lpTwapWindow` misconfig cascade-bricks all NAV reads ✅(SEC-10) · `SzipReservoirLpOracle` monotonic gap · revaluation batch all-or-nothing stales every lien on one bad key ✅(SEC-DOC: intentional WOOF-02 fail-closed; per-key try/catch REJECTED; producer shards) · `fund` base→line-only strands closed-line USDC ✅(SEC-07) · `fund` `balanceOf`-vs-`config` donation-grief DoS ✅(SEC-11) · redemption-queue missing unclaimed-claimable guard · `redeem` missing whole-unit guard + overstated event · NAV-freshness `validTo` edge stale fill ✅(SEC-13) · `postBid` unsatisfiable at `maxAge==0` · RESOLVE/WRITEOFF `ExceedsBond` strands lien in Defaulted ✅(SEC-DOC: NOT a strand — over-bond reverts the whole tx atomically + is re-submittable; assert REJECTED as no-op) · szALPHA sub-rao dust unredeemable · reservoir borrow double-counts coverage (self-brick) ✅(SEC-DOC: re-documented as a fail-closed DOUBLE-squeeze — numerator down + floor up — recovers on repay) · `freeValue` desyncs from `gross−coverageValue` · adapters omit inverse-direction (BaseAdapter convention) ✅(SEC-DOC: intentionally forward-only; reverse already fails closed; inverse support REJECTED as dead code) · registry global scale assumes 18-dp base ✅(SEC-DOC: documented load-bearing — don't relax the 18-dp guard without per-key scale) · USDC→eePool approval not reset ✅(SEC-DOC: settles to 0 — exact-amount approve, EE pulls full, eePool Timelock-only; comment added) · `setOperator` peer-guard divergence (7 of 8 modules) ✅(SEC-15). _Details in the per-pass files._

### INFORMATIONAL / design
Zodiac module mastercopies never init-locked (docstring false; benign — CALL-only) — **RESOLVED 2026-06-16 (SEC-14)** · `SzAlphaRateOracle` "no owner / all knobs immutable" doc inaccurate ✅(SEC-DOC, I4: corrected — `is ReceiverTemplate is Ownable`, Forwarder + identity Timelock-mutable, only economic knobs immutable; `claude-zipcode.md:146` + §8.8 + 8-Bw wire fixed) · par-burn settlement hardcodes 1:1 with no impairment haircut (peg integrity pushed entirely to trust) ✅(SEC-DOC, I3+prorata: documented — single-requester treasury-internal, `MultipleRequesters` is the sole defense, `redeemController` must never be untrusted; the SENIOR queue is intentionally impairment-blind while the JUNIOR NAV self-prices via `writeProvision`) · junior szipUSD has no on-chain exit path (design trade-off) ✅(SEC-DOC, I1: re-affirmed in §6.4 — exit is the NAV-tracking CoW secondary, by design) · szipUSD issuance decoupled from EE share value (zipUSD hard-marked $1) ✅(SEC-DOC, I2: tightened — shares are NAV-priced; flat $1 only on the zipUSD deposit-input leg; zipUSD de-peg over-issue risk noted LOW).
