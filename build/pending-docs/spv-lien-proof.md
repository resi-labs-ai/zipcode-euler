...
> **Disposition:** forward input for **CRE-01** (Proof-of-Lien/Value/Insurance inputs; DEC-01 resolved) + the
> open off-chain/legal legs (SPV custody partner, insurance carrier, per-jurisdiction Proof schema). Collateral
> stays **mocked** for M1 — these are integration tasks, not build blockers (`claude-zipcode.md §15`).
