...
> **Disposition:** forward input for **CRE-05** (vote-floor / regime-classifier / soft-bleed-cap policy, §9.2/§4)
> + **FE-04** (market context). Dies when 8-B11/8-B12 land (`build/tickets/PROGRESS.md` → Deletion triggers). The on-chain engine modules
> (8-B5…8-B10) are BUILT — their spec is `build/wires/` + `claude-zipcode.md §8.7`; older `§4.5.1` refs resolve
> there.
