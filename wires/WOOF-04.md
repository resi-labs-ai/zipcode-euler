...
- `closeLine(lineRef) onlyController` — requires open + `observeDebt == 0` else `LineNotRepaid()`. The escrow shares are owned by `borrowAccount`, so it routes through `evc.call(collateralVault, borrowAccount, 0, redeem(shares, controller, borrowAccount))` (4-arg `IEVC.call`; adapter is the authorized operator). Returns the full `1e18` to the controller; sets `open = false` (record kept readable). Emits `LineClosed`.
...
- **adapter ↔ EE_POOL (allocator + curator).** The adapter holds **both** EE roles. As **curator** it `submitCap`/`acceptCap`s the new line vault inside `openLine` (bounded to its own minted vault). As **allocator** it `setSupplyQueue` (openLine) and `reallocate` (fund). Granting these roles is item-10 (§9), not done in this contract.
