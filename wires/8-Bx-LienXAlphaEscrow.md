...
- **SLASH (built + mock-tested now, M2-live with the `DefaultCoordinator` driver):** on a default the
  ...
  `slashXAlphaToCapital` (xALPHA → `capitalSink`, off-chain alpha→TAO→USDC, covers a realized capital
...
|---|---|
| `LienXAlphaEscrow` (`is ReentrancyGuard, Ownable`) | Holds the per-lien xALPHA bond. Four `onlyCoordinator nonReentrant` state-changers (`lockXAlpha`/`releaseXAlpha`/`slashXAlphaToCapital`/`slashXAlphaToCohort`); four `onlyOwner` wiring setters (`setXAlpha`/`setCoordinator`/`setCapitalSink`/`setSidecar`); per-lien book `bondAmount[lienId]` + `bondOriginator[lienId]`. No recipient param on any transfer. Custody-models `reference/.../InsuranceFund.sol:bring(:33)` (single-authorized-caller + gated `safeTransfer`), replicated clean-room over OZ `IERC20`+`SafeERC20` (not imported). |
...
- **`address public capitalSink`** (`:62`) — the **ONLY** destination of `slashXAlphaToCapital`. Setter
  `setCapitalSink` (`:133`).
...
`WiringSet(bytes32 slot, address value)` (slot = `"xAlpha"`/`"coordinator"`/`"capitalSink"`/`"sidecar"`).
...
- **`slashXAlphaToCapital(bytes32 lienId, uint256 amount)`** (`:188`) — route `amount` (≤ bond) xALPHA to
  `capitalSink`. Guards: `amount != 0` (`ZeroAmount`), `amount <= bondAmount[lienId]` (`ExceedsBond`;
...
`capitalSink` (capital slash), or `sidecar` (cohort slash). The destinations are structurally fixed (per
...
- **`capitalSink` = the off-chain recovery/bridge account** (§11) — receives `slashXAlphaToCapital`; it
  liquidates alpha → TAO → USDC on Bittensor and supplies that USDC to the warehouse to fill the realized
  hole. The escrow only **routes** xALPHA to it (no swap on-chain).
...
- Construct with the **four wiring args** `(xAlpha, coordinator, capitalSink, sidecar)`; ctor reverts
...
- Wire: `xAlpha`→ the bridged xALPHA (8x-01 `SzAlphaMirror`, stand-in until the lane is live);
  ...
  `capitalSink`→ the off-chain recovery/bridge account (alpha→TAO→USDC); `sidecar`→ the 8-B1 substrate
  sidecar Safe.
...
- **Destination integrity (the security thesis).** No state-changer takes a recipient parameter; xALPHA can
  only ever reach `{recorded bondOriginator, capitalSink, sidecar}`. So a **compromised `coordinator`
